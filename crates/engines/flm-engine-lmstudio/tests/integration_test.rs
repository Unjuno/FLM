//! Integration tests for LM Studio engine adapter
//!
//! These tests verify that the LM Studio engine adapter works correctly with mock HTTP servers.
...
#[tokio::test]
async fn test_lmstudio_engine_id() {
    let mock_server = MockServer::start().await;
    let engine = LmStudioEngine::new("lmstudio-test".to_string(), mock_server.uri()).unwrap();

    assert_eq!(engine.id(), "lmstudio-test".to_string());
    assert_eq!(
        engine.kind(),
        flm_core::domain::models::EngineKind::LmStudio
    );
}

#[tokio::test]
async fn test_lmstudio_engine_list_models() {
    let mock_server = MockServer::start().await;
    ...
    assert_eq!(models[0].model_id, "flm://lmstudio-test/llama-2-7b-chat");
    assert_eq!(models[0].display_name, "llama-2-7b-chat");
}

#[tokio::test]
async fn test_lmstudio_engine_health_check() {
    let mock_server = MockServer::start().await;
...
#[tokio::test]
async fn test_lmstudio_engine_chat() {
    let mock_server = MockServer::start().await;
    ...
    let req = ChatRequest {
        engine_id: "lmstudio-test".to_string(),
        model_id: "flm://lmstudio-test/llama-2-7b-chat".to_string(),
...
#[tokio::test]
async fn test_lmstudio_engine_embeddings() {
    let mock_server = MockServer::start().await;
...
#[tokio::test]
async fn test_lmstudio_engine_chat_stream() {
    let mock_server = MockServer::start().await;
