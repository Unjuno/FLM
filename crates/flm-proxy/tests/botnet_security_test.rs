...
async fn test_resource_protection_should_throttle_below_threshold() {
    ...
    let protection = ResourceProtection::new()
        .with_thresholds(1.5, 1.5); // 150% threshold (impossible to exceed)
...
async fn test_resource_protection_status_tracking() {
    ...
    // This is a valid state (either true or false depending on actual system load)
    assert!(
        is_active == true || is_active == false,
        "Protection status should be a boolean"
    );
}
