...
impl ApiPromptStore {
    ...
    pub async fn delete(&self, api_id: &str) -> Result<bool, RepoError> {
        let result = sqlx::query("DELETE FROM api_prompts WHERE api_id = ?")
            .bind(api_id)
            .execute(&self.pool)
            .await
            .map_err(|e| RepoError::IoError {
                reason: format!("Failed to delete API prompt: {e}"),
            })?;
