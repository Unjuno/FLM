...
async fn check_config_db(
        ...
        .await
    {
        ...
        checks.push(CheckItem {
            ...
            status: CheckStatus::Ok,
            message: if verbose {
                Some(format!("{count} settings found"))
            } else {
                None
            },
        });
        ...
        .await
    {
        ...
        checks.push(CheckItem {
            ...
            status: CheckStatus::Ok,
            message: if verbose {
                Some(format!("{count} profiles found"))
            } else {
                None
            },
        });
...
async fn check_security_db(
        ...
        .await
    {
        ...
        checks.push(CheckItem {
            ...
            status: CheckStatus::Ok,
            message: if verbose {
                Some(format!("{count} total keys, {active_count} active"))
            } else {
                None
            },
        });
                ...
                .await
        {
            ...
            if invalid_count > 0 {
                checks.push(CheckItem {
                    ...
                    status: CheckStatus::Error,
                    message: Some(format!("{invalid_count} keys with invalid hash")),
                });
            } else {
                checks.push(CheckItem {
                    name: "security.db api_keys hash integrity".to_string(),
        ...
        .await
    {
        ...
        checks.push(CheckItem {
            ...
            status: CheckStatus::Ok,
            message: if verbose {
                Some(format!("{count} policies found"))
            } else {
                None
            },
        });
