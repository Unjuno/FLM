...
pub struct AppState {
    ...
    /// HTTPS port used for redirects (None in local-http mode)
    pub https_redirect_port: Option<u16>,
    /// Public hostname used for redirects (ACME domain, if configured)
    pub public_base_host: Option<String>,
    /// Metrics collector
...
pub async fn policy_middleware(
    ...
    // 3. Check IP-based rate limit (configurable from policy, default 1000 req/min)
    // This is in addition to API key-based rate limiting
    {
        // Get IP rate limit settings from policy, or use defaults
        let (ip_rate_limit_rpm, ip_rate_limit_burst) =
            if let Some(ip_rate_limit) = policy_json.get("ip_rate_limit") {
        ...
        // Apply dynamic adjustment based on IP's past behavior
        let (adjusted_rpm, adjusted_burst) = adjust_ip_rate_limit_dynamically(
            &state,
            &client_ip,
            ip_rate_limit_rpm,
            ip_rate_limit_burst,
        ...
        let (allowed, _remaining, reset_time) =
            check_ip_rate_limit_with_info(&state, &client_ip, adjusted_rpm, adjusted_burst).await;

        // Persist rate limit state to database asynchronously
        ...
        tokio::spawn(async move {
            // Save IP rate limit state to database
            ...
            {
                error!(
                    error_type = "ip_rate_limit_persist_failed",
                    ip = %utils::mask_ip_address(&client_ip),
                    "Failed to persist IP rate limit state: {}", e
                );
            }
        });
...
pub(crate) fn check_ip_allowed(client_ip: &IpAddr, whitelist_entry: &str) -> bool {
...
/// Adjust IP rate limit dynamically based on IP's past behavior
///
/// Returns (adjusted_rpm, adjusted_burst)
async fn adjust_ip_rate_limit_dynamically(
    state: &AppState,
    ip: &IpAddr,
    base_rpm: u32,
    base_burst: u32,
) -> (u32, u32) {
    // Check if IP has been blocked or has high anomaly score
    let is_blocked = state.ip_blocklist.is_blocked(ip).await;
    let anomaly_score = state.anomaly_detection.get_score(ip).await;
    let intrusion_score = state.intrusion_detection.get_score(ip).await;

    // Reduce rate limit for suspicious IPs
    if is_blocked {
        // Blocked IPs get very low rate limit
        return (10, 10);
    }
...
///
/// Returns (allowed, remaining, reset_time)
pub(crate) async fn check_ip_rate_limit_with_info(
    state: &AppState,
    ip: &IpAddr,
    _rpm: u32,
    burst: u32,
