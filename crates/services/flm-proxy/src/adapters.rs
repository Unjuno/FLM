...
/// Metadata about stored TLS certificates (ACME, packaged, etc.)
#[derive(Clone, Debug)]
...
impl SqliteSecurityRepository {
    ...
    /// Load all active rate limit states from database
    ///
    /// Returns a map of API key ID to (requests_count, reset_at) for all states
    /// where reset_at is in the future.
    pub async fn load_all_active_rate_limit_states(
        &self,
    ) -> Result<std::collections::HashMap<String, (u32, String)>, RepoError> {
        let now = chrono::Utc::now().to_rfc3339();
        let rows = sqlx::query_as::<_, (String, i64, String)>(
            "SELECT api_key_id, requests_count, reset_at FROM rate_limit_states WHERE reset_at > ?",
        )
        .bind(&now)
        .fetch_all(&self.pool)
        .await
        .map_err(|e| RepoError::IoError {
            reason: format!("Failed to load rate limit states: {e}"),
        })?;

        Ok(rows
            .into_iter()
            .map(|(api_key_id, count, reset_at)| (api_key_id, (count as u32, reset_at)))
            .collect())
    }
    ...
    /// Persist certificate metadata (path, expiration, mode)
    pub async fn save_certificate_metadata(
        &self,
        metadata: CertificateMetadata,
    ) -> Result<(), RepoError> {
        let now = chrono::Utc::now().to_rfc3339();
        sqlx::query(
            "INSERT OR REPLACE INTO certificates (id, cert_path, key_path, mode, domain, expires_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
        )
        .bind(&metadata.id)
        .bind(&metadata.cert_path)
        .bind(&metadata.key_path)
        .bind(&metadata.mode)
    ...
    /// Fetch certificate metadata by domain
    ///
    /// Returns the most recent valid certificate for the given domain, if available.
    pub async fn fetch_certificate_metadata_by_domain(
        &self,
        domain: &str,
    ) -> Result<Option<CertificateMetadata>, RepoError> {
        let row = sqlx::query_as::<_, (String, String, String, String, Option<String>, Option<String>)>(
            "SELECT id, cert_path, key_path, mode, domain, expires_at FROM certificates WHERE domain = ? AND mode = 'acme' ORDER BY updated_at DESC LIMIT 1",
        )
        .bind(domain)
        .fetch_optional(&self.pool)
        .await
        .map_err(|e| RepoError::IoError {
            reason: format!("Failed to fetch certificate metadata: {e}"),
        })?;

        if let Some((id, cert_path, key_path, mode, domain, expires_at)) = row {
            Ok(Some(CertificateMetadata {
                id,
                cert_path,
                key_path,
                mode,
