...
mod tests {
    ...
    #[tokio::test]
    async fn test_ip_blocklist_new() {
    ...
    #[tokio::test]
    async fn test_ip_blocklist_record_failure_warning() {
        let blocklist = IpBlocklist::new();
        let ip = IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1));

        // 1-4 failures should only warn
        for _ in 0..4 {
            let should_block = blocklist.record_failure(ip).await;
            assert!(!should_block, "1-4 failures should not block");
            assert!(!blocklist.is_blocked(&ip).await);
        }
    }

    #[tokio::test]
    async fn test_ip_blocklist_record_failure_30min_block() {
        let blocklist = IpBlocklist::new();
        let ip = IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1));

        // 5 failures should trigger 30-minute block
        for _ in 0..5 {
            blocklist.record_failure(ip).await;
        }

        let should_block = blocklist.record_failure(ip).await;
        assert!(should_block, "5 failures should trigger block");
        assert!(blocklist.is_blocked(&ip).await);
    }

    #[tokio::test]
    async fn test_ip_blocklist_record_failure_24h_block() {
        let blocklist = IpBlocklist::new();
        let ip = IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1));

        // 10 failures should trigger 24-hour block
        for _ in 0..10 {
            blocklist.record_failure(ip).await;
        }
    ...
    #[tokio::test]
    async fn test_ip_blocklist_record_failure_permanent_block() {
        let blocklist = IpBlocklist::new();
        let ip = IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1));

        // 20 failures should trigger permanent block
        for _ in 0..20 {
            blocklist.record_failure(ip).await;
        }
    ...
    #[tokio::test]
    async fn test_ip_blocklist_clear_temporary_blocks() {
        let blocklist = IpBlocklist::new();
        ...
        for _ in 0..5 {
            blocklist.record_failure(ip1).await;
        }

        // Create permanent block
        for _ in 0..20 {
            blocklist.record_failure(ip2).await;
        }

        assert!(blocklist.is_blocked(&ip1).await);
        assert!(blocklist.is_blocked(&ip2).await);

        // Clear temporary blocks
        blocklist.clear_temporary_blocks().await;

        // Temporary block should be cleared
        assert!(!blocklist.is_blocked(&ip1).await);
        // Permanent block should remain
        assert!(blocklist.is_blocked(&ip2).await);
    }

    #[tokio::test]
    async fn test_ip_blocklist_needs_sync() {
        let blocklist = IpBlocklist::new();
        assert!(!blocklist.needs_sync().await);

        // Wait for sync interval (5 minutes) - but we'll just check the logic
        // In a real test, we might use a mock or shorter interval
        blocklist.mark_synced().await;
        assert!(!blocklist.needs_sync().await);
    }

    #[tokio::test]
    async fn test_ip_blocklist_get_blocked_ips() {
        let blocklist = IpBlocklist::new();
        let ip1 = IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1));
        let ip2 = IpAddr::V4(Ipv4Addr::new(127, 0, 0, 2));
        ...
        let blocked = blocklist.get_blocked_ips().await;
