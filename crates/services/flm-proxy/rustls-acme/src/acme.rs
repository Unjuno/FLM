...
impl Account {
    ...
    pub fn dns_01<'a>(&self, challenges: &'a [Challenge]) -> Result<(&'a Challenge, String), AcmeError> {
        ...
        let digest = key_authorization_sha256(&self.key_pair, &challenge.token)?;
        Ok((challenge, BASE64_URL_SAFE_NO_PAD.encode(digest.as_ref())))
    }
}
...
mod tests {
    ...
    fn test_account() -> Account {
        let key_bytes = Account::generate_key_pair();
        let key_pair = EcdsaKeyPair::from_pkcs8(
            ALG,
            &key_bytes,
            #[cfg(all(feature = "ring", not(feature = "aws-lc-rs")))]
    ...
    #[test]
    fn dns01_returns_expected_value() {
        let account = test_account();
        let token = "sample-token";
        let challenges = vec![Challenge {
            typ: ChallengeType::Dns01,
            url: "https://example.com/chal".into(),
            token: token.into(),
            error: None,
        }];

        let (_, value) = account.dns_01(&challenges).expect("dns-01 challenge");
        let digest = key_authorization_sha256(&account.key_pair, token).unwrap();
        let expected = BASE64_URL_SAFE_NO_PAD.encode(digest.as_ref());
        assert_eq!(value, expected);
    }

    #[test]
    fn dns01_missing_challenge_errors() {
        let account = test_account();
        let challenges = vec![Challenge {
            typ: ChallengeType::Http01,
            url: "https://example.com/chal".into(),
            token: "token".into(),
            error: None,
        }];
