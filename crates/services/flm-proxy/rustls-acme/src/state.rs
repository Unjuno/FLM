...
use crate::dns::{DnsChallengeHook, DnsChallengeRecord};
...
pub enum OrderError {
    ...
    #[error("dns challenge hook is not configured")]
    DnsHookMissing,
    #[error("dns challenge hook error: {0}")]
    DnsHook(String),
}
...
impl<EC: 'static + Debug, EA: 'static + Debug> AcmeState<EC, EA> {
    ...
    async fn authorize(config: &AcmeConfig<EC, EA>, resolver: &ResolvesServerCertAcme, account: &Account, url: &String) -> Result<(), OrderError> {
        ...
        let (domain, challenge_url, dns_record) = match auth.status {
            AuthStatus::Pending => {
                let Identifier::Dns(domain) = auth.identifier;
                log::info!("trigger challenge for {}", &domain);
                let mut dns_record: Option<DnsChallengeRecord> = None;
                let challenge = match config.challenge_type {
                    ...
                    UseChallenge::Dns01 => {
                        let hook = config.dns_hook.as_ref().ok_or(OrderError::DnsHookMissing)?.clone();
                        ...
                        hook.present(&record).await.map_err(|err| OrderError::DnsHook(err.to_string()))?;
                        resolver.set_dns_01_challenge_data(record.fqdn.clone(), record.value.clone());
                        dns_record = Some(record);
                        challenge
                    }
                };
                account.challenge(&config.client_config, &challenge.url).await?;
                (domain, challenge.url.clone(), dns_record)
            }
            AuthStatus::Valid => {
        ...
        for i in 0u64..5 {
            ...
            match auth.status {
                ...
                AuthStatus::Valid => {
                    // clear challenge data when auth validated
                    Self::cleanup_dns_record(&config.dns_hook, dns_record.as_ref()).await;
                    resolver.clear_challenge_data();
                    return Ok(());
                }
                _ => {
    ...
    async fn cleanup_dns_record(hook: &Option<Arc<dyn DnsChallengeHook>>, record: Option<&DnsChallengeRecord>) {
        if let (Some(hook), Some(record)) = (hook, record) {
            if let Err(err) = hook.cleanup(record).await {
                log::warn!("failed to clean up DNS-01 TXT record {} ({}): {}", record.fqdn, record.domain, err);
            }
        }
    }
