...
enum ChallengeData {
    ...
    Dns01 { fqdn: String, value: String },
}
...
impl ResolvesServerCertAcme {
    ...
    pub(crate) fn set_dns_01_challenge_data(&self, fqdn: String, value: String) {
        self.inner.lock().unwrap().challenge_data = Some(ChallengeData::Dns01 { fqdn, value });
    }
    ...
    pub fn get_dns_01_record(&self) -> Option<(String, String)> {
        match &self.inner.lock().unwrap().challenge_data {
            Some(ChallengeData::Dns01 { fqdn, value }) => Some((fqdn.clone(), value.clone())),
            _ => None,
        }
    }
}
