...
//! The validation mechanism used is TLS-ALPN-01, HTTP-01, or DNS-01 which allow serving acme challenge responses and
//! regular TLS traffic on the same port (or through DNS automation).
//!
...
//!
//! TLS-ALPN-01 remains the recommended default.
//! However, if the DNS entry points to an HTTP proxy, which terminates TLS, but you still need a certificate,
//! you may be able to use HTTP-01 instead (see `examples/low_level_axum_http01.rs`) or automate DNS-01
//! via [`DnsChallengeHook`] to integrate with a DNS provider.
//!
//! ## High-level API
//!
