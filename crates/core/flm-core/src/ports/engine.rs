...
pub trait LlmEngine: Send + Sync {
    ...
    ///
    /// This method is optional and should only be implemented by engines
    /// that support audio transcription (e.g., Whisper models).
    /// Engines that don't support transcription should return
    /// `EngineError::UnsupportedOperation`.
    async fn transcribe_audio(
        &self,
        _req: TranscriptionRequest,
    ) -> Result<TranscriptionResponse, EngineError> {
        // Default implementation returns unsupported operation
        Err(EngineError::UnsupportedOperation {
            operation: "transcribe_audio".to_string(),
            reason: "Engine does not support audio transcription".to_string(),
        })
    }
}
