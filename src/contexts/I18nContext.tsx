...
export const I18nProvider: React.FC<I18nProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    const loadLocale = async () => {
      ...
      try {
        const settings = await safeInvoke<{ language?: string }>('get_app_settings');
  ...
  // 翻訳値を取得するヘルパー関数
  const getTranslationValue = useCallback((translationObj: Translations, keys: string[]): string | null => {
  ...
  const setLocale = useCallback(async (newLocale: Locale) => {
    ...
    try {
      const currentSettings = await safeInvoke<{ language?: string }>('get_app_settings');
      ...
      // invokeが未定義の場合の特別な処理
      if (errorMessage.includes('invoke') || errorMessage.includes('undefined') || errorMessage.includes('Cannot read properties') || errorMessage.includes('アプリケーションが正しく起動')) {
        console.warn('Tauri環境が初期化されていません。言語設定を保存できません');
      } else {
