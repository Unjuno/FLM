...
export function generateApiKey(
  length: number = API_KEY.DEFAULT_LENGTH
): string {
  if (length < API_KEY.MIN_LENGTH) {
    throw new Error(
      `APIキーの長さは${API_KEY.MIN_LENGTH}文字以上である必要があります。`
    );
  }
 ...
 * 監査ログと同じロジックを使用（SHA-256の最初の8バイトのみ使用）
 * @param ipAddress IPアドレス
 * @returns ハッシュ値（16文字の16進数文字列、プレフィックス付き）
 */
export function hashIpAddress(ipAddress: string): string {
  const hash = crypto.createHash('sha256').update(ipAddress).digest('hex');
  // 最初の8バイト（16文字）のみを使用（監査ログと同じロジック）
 ...
 */
async function getApiKeyHashFromDatabase(
  apiKey: string
): Promise<string | null> {
  try {
  ...
  try {
    ...
    if (normalizedApiKeyHash.length !== normalizedStoredHash.length) {
      ...
      crypto.timingSafeEqual(
        ...
        Buffer.from(
          normalizedStoredHash.substring(0, normalizedApiKeyHash.length)
        )
      );
      return false;
    }
