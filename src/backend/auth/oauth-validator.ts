    ...
    try {
        ...
        return {
            ...
            tokenInfo: {
                ...
                expires_at: data.exp ? new Date(data.exp * 1000).toISOString() : undefined,
        ...
        return {
            valid: false,
            error: error instanceof Error ? error.message : 'イントロスペクションエラー'
        };
    }
}
    ...
    try {
        ...
        return {
            ...
            tokenInfo: {
                ...
                scope: payload.scope ? (typeof payload.scope === 'string' ? payload.scope.split(' ') : payload.scope) : [],
...
export async function validateOAuthToken(
    ...
    if (config?.introspectionEndpoint && config?.clientId && config?.clientSecret) {
