...
export const CertificateManagement: React.FC = () => {
  ...
  const loadCertificates = async () => {
    try {
      ...
      // 証明書一覧を取得
      const certificatesData = await safeInvoke<CertificateInfo[]>('get_all_certificates', {});
      setCertificates(certificatesData);
    } catch (err) {
      // エラーは静かに処理（基盤実装のため）
      if (process.env.NODE_ENV === 'development') {
        // eslint-disable-next-line no-console
        console.error('証明書の読み込みに失敗しました:', err);
      }
      setCertificates([]);
    } finally {
      setLoading(false);
  ...
  const handleRenewCertificate = async (_domain: string) => {
    try {
      ...
      showError(err instanceof Error ? err.message : '証明書の更新に失敗しました');
    } finally {
      setRenewing(false);
    }
  };
      ...
      <div className="certificate-management-container">
        ...
        <div className="certificate-management-content">
          <div className="certificate-info-banner">
            ...
            <p>
              SSL証明書の管理ができます。Let's Encryptを使用した証明書の自動取得・更新機能の基盤が実装されています。
            </p>
              ...
              certificates.map((cert, index) => (
                <div key={index} className="certificate-item">
                  <div className="certificate-header">
                    ...
                    <span className={`certificate-status ${getCertificateStatusClass(cert)}`}>
                  ...
                  <div className="certificate-details">
                    <div className="certificate-detail-row">
                      ...
                      <span className="certificate-detail-value">{cert.issuer}</span>
                    </div>
                    <div className="certificate-detail-row">
                      <span className="certificate-detail-label">有効期限開始:</span>
                      <span className="certificate-detail-value">{cert.valid_from}</span>
                    </div>
                    <div className="certificate-detail-row">
                      <span className="certificate-detail-label">有効期限終了:</span>
                      <span className="certificate-detail-value">{cert.valid_to}</span>
                    </div>
                  </div>
