...
export const ApiTestSelector: React.FC = () => {
  ...
  const { t } = useI18n();
  const [apis, setApis] = useState<ApiInfoSimple[]>([]);
  ...
  const loadApis = useCallback(async () => {
    ...
    try {
      ...
      const apiInfos: ApiInfoSimple[] = result.map(api => ({
        id: api.id,
        name: api.name,
      ...
      setError(
        err instanceof Error ? err.message : t('apiTestSelector.messages.loadError')
      );
