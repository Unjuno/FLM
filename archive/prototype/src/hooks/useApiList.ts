...
export const useApiList = () => {
  ...
  const loadApis = useCallback(async (force: boolean = false) => {
    ...
    try {
        ...
        result.map(async (api) => {
          ...
          if (status === 'running') {
            try {
              ...
              proxy_running = false;
              logger.debug(`プロキシヘルスチェックエラー (API: ${api.name}, ポート: ${api.port}):`, err, 'useApiList');
            }
          }
      ...
      logger.error(currentT('apiList.messages.loadError'), toError(err), 'useApiList');
    } finally {
      // アンマウントチェック
      if (isMounted()) {
        setLoading(false);
  ...
  // ポーリング用のラッパー関数
  const pollLoadApis = useCallback(async (options?: { force?: boolean }) => {
