...
export const useApiOperations = (loadApis: () => Promise<void>) => {
  ...
  // API起動/停止中の状態を管理
  const [operatingApiIds, setOperatingApiIds] = useState<Set<string>>(
    new Set()
  );
  // API操作の進捗情報を管理
  const [apiOperationProgress, setApiOperationProgress] = useState<
    Map<string, ApiOperationProgress>
  >(new Map());
  ...
  const toggleApiStatus = useCallback(
    async (
      ...
      try {
        ...
        try {
          unsubscribeProgressRef.current =
            await listen<ApiOperationProgressEvent>(
        ...
        const errorMessage = extractErrorMessage(
          err,
          t('apiList.messages.statusChangeError')
        );
        if (isMounted()) {
