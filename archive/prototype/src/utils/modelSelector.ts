...
export async function selectBestModel(
  ...
  try {
    ...
    for (const model of webModelConfig.models) {
      ...
      if (
        ...
        if (
          ...
          score += 15;
          reasons.push(
            `メモリ要件を満たす（${model.requirements.minMemory}GB以上必要、利用可能: ${requirements.availableMemory}GB）`
          );

          // 推奨メモリを満たしている場合は追加点
          if (
            model.requirements.recommendedMemory &&
          ...
          score -= 20; // メモリ不足は大幅減点
          reasons.push(
            `メモリ不足（必要: ${model.requirements.minMemory}GB、利用可能: ${requirements.availableMemory}GB）`
          );
        }
      }
      ...
      // GPU要件マッチング（5点）
      if (
        requirements.hasGpu !== undefined &&
        model.requirements?.gpuRecommended !== undefined
      ) {
      ...
      if (
        ...
        if (isFinite(modelSizeGB) && modelSizeGB > requirements.maxModelSize) {
          score -= 30;
          reasons.push(
            `モデルサイズが制限を超える（${modelSizeGB.toFixed(FORMATTING.DECIMAL_PLACES_SHORT)}GB > ${requirements.maxModelSize}GB）`
          );
        }
      }
    ...
    if (
      ...
      throw new Error(
        `モデル "${selectedModel.name || '不明'}" に必須プロパティが設定されていません`
      );
    }
    ...
    if (!selectedModel.defaultSettings) {
      throw new Error(
        `モデル "${selectedModel.name}" にdefaultSettingsが設定されていません`
      );
    }

    // ポート番号の妥当性チェック
    let port =
      requirements.preferredPort ??
      selectedModel.defaultSettings.port ??
      PORT_RANGE.DEFAULT;
    if (
      typeof port !== 'number' ||
      !isFinite(port) ||
      ...
      if (isDev()) {
        console.warn(
          `無効なポート番号: ${port}。デフォルト値${PORT_RANGE.DEFAULT}を使用します。`
        );
      }
    ...
    const apiConfig: ModelSelectionResult['config'] = {
      ...
      enableAuth:
        requirements.enableAuth ??
        selectedModel.defaultSettings.enableAuth ??
        true,
...
export async function getSystemResources(): Promise<{
  ...
  try {
    ...
    let availableMemoryGB =
      resources.available_memory / FORMATTING.BYTES_PER_GB;

    // NaNや負の値のチェック
    if (!isFinite(availableMemoryGB) || availableMemoryGB < 0) {
      if (isDev()) {
        console.warn(
          '利用可能メモリの値が不正です。デフォルト値を使用します。',
          availableMemoryGB
        );
      }
    ...
    if (
      ...
      error.message.includes('Tauri環境が初期化されていません')
    ) {
      throw error;
    }
