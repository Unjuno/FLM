...
pub async fn request_certificate(domain: &str, contact_email: &str) -> Result<()> {
    ...
    // NOTE: acme-clientの証明書取得処理の実装が必要です
    // 実装時は以下の点に注意してください:
    // - HTTP-01チャレンジのトークンを提供する仕組みが必要
    // - 証明書ファイルと秘密鍵の保存先がアプリケーション設定と一致していること
    // - エラーハンドリングとログ記録を適切に実装すること
}`,
        checklist: [
...
pub async fn issue_certificate(domain: String, email: String, app_state: State<'_, AppState>) -> Result<(), String> {
    ...
    // NOTE: 証明書保存後の再読み込みや設定反映の処理が必要です
    // 実装時は以下の点に注意してください:
    // - 証明書保存後にサーバー設定を再読み込みすること
    // - 設定変更をユーザーに通知すること
    // - エラー発生時のロールバック処理を実装すること
    Ok(())
}`,
        checklist: [
