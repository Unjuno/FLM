...
export const Diagnostics: React.FC = () => {
  ...
  const [ollamaHealth, setOllamaHealth] = useState<OllamaHealthStatus | null>(
    null
  );
  const [securityBlock, setSecurityBlock] =
    useState<SecurityBlockDetection | null>(null);
  ...
  const [filesystemDiagnostics, setFilesystemDiagnostics] =
    useState<FilesystemDiagnostics | null>(null);
  const [databaseIntegrity, setDatabaseIntegrity] =
    useState<DatabaseIntegritySummary | null>(null);
  const [comprehensiveDiagnostics, setComprehensiveDiagnostics] =
    useState<ComprehensiveDiagnostics | null>(null);
  ...
  const runDiagnostics = useCallback(async (forceRefresh = false) => {
    ...
    if (!forceRefresh && diagnosticCacheRef.current) {
      ...
      if (cacheAge < CACHE_DURATION) {
        ...
        if (cached.comprehensive) {
          ...
          if (cached.comprehensive.security)
            setSecurityBlock(cached.comprehensive.security);
          if (cached.comprehensive.network)
            setNetworkDiagnostics(cached.comprehensive.network);
          if (cached.comprehensive.environment)
            setEnvironmentDiagnostics(cached.comprehensive.environment);
          if (cached.comprehensive.filesystem)
            setFilesystemDiagnostics(cached.comprehensive.filesystem);
          if (cached.comprehensive.resources)
            setSystemResources(cached.comprehensive.resources);
        }
    ...
    try {
      ...
      const comprehensiveResult = await safeInvoke<ComprehensiveDiagnostics>(
      ...
      // フェーズ3: データベース診断（中速）
      setDiagnosticProgress({
        current: 'データベース整合性をチェック中...',
        completed: 2,
        total: 6,
      });
      const databaseResult = await safeInvoke<DatabaseIntegritySummary>(
        'check_database_integrity'
      ).catch(err => {
        logger.warn(
          'データベース整合性チェックに失敗しました',
          extractErrorMessage(err),
          'Diagnostics'
        );
        return null;
        ...
        const portCheckEntries = await Promise.all(
          mappedApis.map(async api => {
            try {
              ...
              return [
                api.id,
                { port: api.port, available } as PortCheckResult,
              ] as const;
            } catch (err) {
      ...
      const [enginesResult, modelsResult, pluginsResult, tasksResult] =
        await Promise.all([
          safeInvoke<EngineDetectionResult[]>('detect_all_engines').catch(
            err => {
              logger.warn(
                'エンジン検出に失敗しました',
                extractErrorMessage(err),
          ...
          safeInvoke<InstalledModelInfo[]>('get_installed_models').catch(
            err => {
              logger.warn(
                'インストール済みモデル取得に失敗しました',
                extractErrorMessage(err),
                'Diagnostics'
              );
              return [];
  ...
  useEffect(() => {
    ...
    logger.debug(
      '診断ツール: useEffect実行',
      `lastChecked=${lastChecked}, checking=${checking}`,
      'Diagnostics'
    );
    if (!lastChecked && !checking && !checkingRef.current) {
      ...
      logger.debug(
        '診断ツール: 初回診断をスキップ',
        `lastChecked=${!!lastChecked}, checking=${checking}, checkingRef=${checkingRef.current}`,
        'Diagnostics'
      );
    }
  ...
  const handleFixDatabase = useCallback(async () => {
    ...
    try {
      const result = await safeInvoke<DatabaseIntegritySummary>(
        'fix_database_integrity'
      );
      setDatabaseIntegrity(result);
      logger.info(
        'データベース整合性の修正が完了しました',
        `問題数: ${result.total_issues}件`,
        'Diagnostics'
      );

      // 修正後、再度診断を実行（強制更新）
      logger.debug(
        '診断ツール: データベース修正後、診断を再実行',
        '',
        'Diagnostics'
      );
        ...
        <div className="diagnostics-header">
          <div className="diagnostics-header-left">
            ...
            <div className="diagnostics-header-text">
              <h1>診断ツール</h1>
              <p>
                アプリケーションの主要コンポーネントとポート使用状況を確認します。
              </p>
            </div>
          </div>
            ...
            <button
              ...
              onClick={() => {
                if (checking || checkingRef.current) {
                  logger.debug(
                    '診断ツール: リフレッシュボタンクリック（既に診断中のためスキップ）',
                    '',
                    'Diagnostics'
                  );
                  return;
            ...
            {diagnosticProgress ? (
              <div className="diagnostics-progress">
                <div className="diagnostics-progress-text">
                  {diagnosticProgress.current} ({diagnosticProgress.completed}/
                    ...
                    ref={el => {
                      if (el) {
                        el.style.setProperty(
                          '--progress-width',
                          `${(diagnosticProgress.completed / diagnosticProgress.total) * 100}%`
                        );
                      }
                    }}
                  />
          ...
          <div className="diagnostics-content">
            {comprehensiveDiagnostics && (
              <section className="diagnostics-section">
                <h2>診断サマリー</h2>
                <div
                  className={`diagnostics-card comprehensive-summary ${comprehensiveDiagnostics.overall_health}`}
                >
                  <div className="summary-header">
                    <h3>
                      総合的な健康状態:
                      <span
                        className={`health-status ${comprehensiveDiagnostics.overall_health}`}
                      >
                        {comprehensiveDiagnostics.overall_health === 'healthy'
                          ? '正常'
                          : comprehensiveDiagnostics.overall_health ===
                              'warning'
                            ? '警告'
                            : '重大'}
                      </span>
                  ...
                  {comprehensiveDiagnostics.critical_issues.length > 0 && (
                    <div className="critical-issues">
                      <h4>優先度の高い問題</h4>
                      <ul>
                        {comprehensiveDiagnostics.critical_issues.map(
                          (issue, idx) => (
                            <li key={idx}>{issue}</li>
                          )
                        )}
            ...
            <section className="diagnostics-section">
              ...
              {securityBlock ? (
                <div className="diagnostics-card">
                  <div
                    className={`diagnostics-status-grid ${securityBlock.likely_blocked ? 'security-warning' : ''}`}
                  >
                    <div>
                      <span className="label">バックエンド応答</span>
                      <span
                        className={
                          securityBlock.backend_responding
                            ? 'status success'
                            : 'status error'
                        }
                      >
                    ...
                    <div>
                      ...
                      <span
                        className={
                          securityBlock.port_1420_listening
                            ? 'status success'
                            : 'status error'
                        }
                      >
                    ...
                    <div className="diagnostics-issues">
                      ...
                      <ul>
                        {securityBlock.issues.map((issue, idx) => (
                          <li
                            key={`security-issue-${idx}-${issue.substring(0, 20)}`}
                          >
                            {issue}
                          </li>
                        ))}
                    ...
                    <div className="diagnostics-recommendations">
                      ...
                      <ul>
                        {securityBlock.recommendations.map((rec, idx) => (
                          <li
                            key={`security-rec-${idx}-${rec.substring(0, 20)}`}
                          >
                            {rec}
                          </li>
                        ))}
                ...
                <div className="diagnostics-card">
                  <div className="diagnostics-status-grid">
                    ...
                    <div>
                      ...
                      <span
                        className={
                          ollamaStatus.portable ? 'status info' : 'status muted'
                        }
                      >
                    ...
                    {apis.map(api => {
                      ...
                      return (
                        <div key={api.id} className="table-row">
                          ...
                          <span>
                            <span className={`status ${portStatus.status}`}>
                              {portStatus.label}
                            </span>
                ...
                <div className="diagnostics-card">
                  <div className="diagnostics-status-grid">
                    <div>
                      ...
                      <span className="value">
                        {formatBytesToGB(systemResources.total_memory)} GB
                      </span>
                    </div>
                    <div>
                      <span className="label">メモリ空き</span>
                      <span className="value">
                        {formatBytesToGB(systemResources.available_memory)} GB
                      </span>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <span className="value">
                        {formatBytesToGB(systemResources.total_disk)} GB
                      </span>
                    </div>
                    <div>
                      <span className="label">ディスク空き</span>
                      <span className="value">
                        {formatBytesToGB(systemResources.available_disk)} GB
                      </span>
                    </div>
                    <div>
                      <span className="label">リソースレベル</span>
                      <span className="status info">
                        {getResourceLevelLabel(systemResources.resource_level)}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="diagnostics-card">
                  <div className="diagnostics-status-grid">
                    <div>
                      ...
                      <span
                        className={
                          networkDiagnostics.internet_available
                            ? 'status success'
                            : 'status error'
                        }
                      >
                    ...
                    <div>
                      ...
                      <span
                        className={
                          networkDiagnostics.dns_resolution
                            ? 'status success'
                            : 'status error'
                        }
                      >
                    ...
                    <div>
                      ...
                      <span
                        className={
                          networkDiagnostics.local_network
                            ? 'status success'
                            : 'status warning'
                        }
                      >
                    ...
                    <div className="diagnostics-issues">
                      ...
                      <ul>
                        {networkDiagnostics.issues.map((issue, idx) => (
                          <li
                            key={`network-issue-${idx}-${issue.substring(0, 20)}`}
                          >
                            {issue}
                          </li>
                        ))}
                    ...
                    <div className="diagnostics-recommendations">
                      ...
                      <ul>
                        {networkDiagnostics.recommendations.map((rec, idx) => (
                          <li
                            key={`network-rec-${idx}-${rec.substring(0, 20)}`}
                          >
                            {rec}
                          </li>
                        ))}
                ...
                <div className="diagnostics-card">
                  <div className="diagnostics-status-grid">
                    <div>
                      ...
                      <span className="value">
                        {environmentDiagnostics.os_info}
                      </span>
                    </div>
                    <div>
                      <span className="label">OSバージョン</span>
                      <span className="value">
                        {environmentDiagnostics.os_version || '不明'}
                      </span>
                    </div>
                    <div>
                      <span className="label">アーキテクチャ</span>
                      <span className="value">
                        {environmentDiagnostics.architecture}
                      </span>
                    </div>
                    <div>
                      <span className="label">Rustバージョン</span>
                      <span className="value">
                        {environmentDiagnostics.rust_version ||
                          '未検出（開発環境のみ）'}
                      </span>
                    </div>
                  </div>
                    ...
                    <div className="diagnostics-issues">
                      ...
                      <ul>
                        {environmentDiagnostics.issues.map((issue, idx) => (
                          <li
                            key={`env-issue-${idx}-${issue.substring(0, 20)}`}
                          >
                            {issue}
                          </li>
                        ))}
                ...
                <div className="diagnostics-card">
                  <div className="diagnostics-status-grid">
                    <div>
                      ...
                      <span
                        className={
                          filesystemDiagnostics.write_permission
                            ? 'status success'
                            : 'status error'
                        }
                      >
                    ...
                    <div>
                      ...
                      <span
                        className={
                          filesystemDiagnostics.data_directory_writable
                            ? 'status success'
                            : 'status error'
                        }
                      >
                    ...
                    <div>
                      ...
                      <span
                        className={
                          filesystemDiagnostics.temp_directory_writable
                            ? 'status success'
                            : 'status error'
                        }
                      >
                    ...
                    <div>
                      ...
                      <span
                        className={
                          filesystemDiagnostics.disk_space_sufficient
                            ? 'status success'
                            : 'status warning'
                        }
                      >
                        {filesystemDiagnostics.available_disk_gb.toFixed(1)} GB
                        利用可能
                      </span>
                    </div>
                  </div>
                    ...
                    <div className="diagnostics-issues">
                      ...
                      <ul>
                        {filesystemDiagnostics.issues.map((issue, idx) => (
                          <li key={`fs-issue-${idx}-${issue.substring(0, 20)}`}>
                            {issue}
                          </li>
                        ))}
                      </ul>
                  ...
                  {filesystemDiagnostics.recommendations.length > 0 && (
                    <div className="diagnostics-recommendations">
                      <h3>推奨される対処法</h3>
                      <ul>
                        {filesystemDiagnostics.recommendations.map(
                          (rec, idx) => (
                            <li key={`fs-rec-${idx}-${rec.substring(0, 20)}`}>
                              {rec}
                            </li>
                          )
                        )}
                ...
                <div className="diagnostics-card">
                  <div className="diagnostics-status-grid">
                    ...
                    <div>
                      ...
                      <span className="value">
                        {databaseIntegrity.total_issues}件
                      </span>
                    </div>
                  </div>
                    ...
                    <div className="database-checks">
                      ...
                      <div className="check-list">
                        {databaseIntegrity.checks.map((check, idx) => (
                          <div
                            key={`db-check-${idx}-${check.check_name}`}
                            className={`check-item ${check.is_valid ? 'valid' : 'invalid'}`}
                          >
                            <div className="check-header">
                              ...
                              <span
                                className={`status ${check.is_valid ? 'success' : 'error'}`}
                              >
                                {check.is_valid ? '✓' : '✗'}
                              </span>
                            </div>
                            ...
                            {check.issues.length > 0 && (
                              <ul className="check-issues">
                                {check.issues.map((issue, issueIdx) => (
                                  <li
                                    key={`db-check-${idx}-issue-${issueIdx}-${issue.substring(0, 20)}`}
                                  >
                                    {issue}
                                  </li>
                                ))}
                ...
                <div className="diagnostics-card">
                  <div className="diagnostics-table">
                    ...
                    {allEngines.map((engine, idx) => (
                      <div
                        key={`engine-${idx}-${engine.engine_type}`}
                        className="table-row"
                      >
                        <span>
                          <strong>
                        ...
                        <span>
                          <span
                            className={`status ${engine.installed ? 'success' : 'error'}`}
                          >
                            {engine.installed
                              ? 'インストール済み'
                              : '未インストール'}
                        ...
                        <span>
                          <span
                            className={`status ${engine.running ? 'success' : 'warning'}`}
                          >
                    ...
                    <div className="diagnostics-issues">
                      ...
                      <ul>
                        {allEngines
                          ...
                          .map((engine, idx) => (
                            <li
                              key={`engine-issue-${idx}-${engine.engine_type}`}
                            >
                    ...
                    {installedModels.map(model => (
                      <div key={model.id} className="table-row">
                        ...
                        <span>
                          <span className="value">
                            {model.engine_type === 'ollama'
                              ? 'Ollama'
                              : model.engine_type === 'lm_studio'
                                ? 'LM Studio'
                                : model.engine_type === 'vllm'
                                  ? 'vLLM'
                                  : model.engine_type === 'llama_cpp'
                                    ? 'llama.cpp'
                                    : model.engine_type}
                          </span>
                        </span>
                        ...
                        <span>
                          <span className="value">
                            {model.installed_at
                              ? new Date(model.installed_at).toLocaleString(
                                  'ja-JP'
                                )
                              : '不明'}
                          </span>
            ...
            <section className="diagnostics-section">
              ...
              {plugins.length === 0 ? (
                ...
                <div className="diagnostics-card">
                  <div className="diagnostics-table">
                    ...
                    {plugins.map(plugin => (
                      <div key={plugin.id} className="table-row">
                          ...
                          <span className="value">
                            {plugin.plugin_type === 'Engine'
                              ...
                              : plugin.plugin_type === 'Model'
                                ? 'モデル'
                                : plugin.plugin_type === 'Auth'
                                  ? '認証'
                                  : plugin.plugin_type === 'Logging'
                                    ? 'ログ'
                                    : plugin.plugin_type}
                          </span>
                        </span>
                        <span>
                          <span
                            className={`status ${plugin.enabled ? 'success' : 'muted'}`}
                          >
                  ...
                  <div className="diagnostics-summary">
                    <p>
                      合計: {plugins.length}個のプラグイン（有効:{' '}
                      {plugins.filter(p => p.enabled).length}個、無効:{' '}
                      {plugins.filter(p => !p.enabled).length}個）
                    </p>
                  </div>
                </div>
              )}
            ...
            <section className="diagnostics-section">
              ...
              {scheduleTasks.length === 0 ? (
                ...
                <div className="diagnostics-card">
                  <div className="diagnostics-table">
                    ...
                    {scheduleTasks.map(task => (
                      <div key={task.id} className="table-row">
                        ...
                        <span>
                          <span
                            className={`status ${task.enabled ? 'success' : 'muted'}`}
                          >
                            {task.enabled ? '有効' : '無効'}
                          </span>
                        </span>
                  ...
                  <div className="diagnostics-summary">
                    <p>
                      合計: {scheduleTasks.length}個のタスク（有効:{' '}
                      {scheduleTasks.filter(t => t.enabled).length}個、無効:{' '}
                      {scheduleTasks.filter(t => !t.enabled).length}個）
                    </p>
                  </div>
                </div>
              )}
            ...
            <section className="diagnostics-section">
              ...
              <div className="diagnostics-card">
                <ul className="diagnostics-tips">
                  <li>
                    ポートが占有されている場合は、ExpressVPN
                    などの常駐アプリが同じポートを使用していないか確認し、必要に応じて停止してください。
                  </li>
                  <li>
                    API が停止中にもかかわらずポートが使用中の場合は、OS
                    を再起動するか{' '}
                    <code>
                  ...
                  <li>
                    Ollama
                    が未インストールの場合は、「Ollamaセットアップ」ページからインストールを行ってください。
                  </li>
                  <li>
                    ダウンロードエラーが発生する場合は、インターネット接続と空き容量（推奨
                    10GB
                    以上）を確認し、必要に応じて別ドライブを指定してください。
                  </li>
                  <li>
                    セキュリティソフトが `ollama.exe` や `flm.exe`
                    をブロックしている場合は、許可リストに追加してから再実行してください。
                  </li>
                  <li>
                    システムリソースが不足している場合は、モデルの縮小や同時実行数の調整を検討してください。
                  </li>
                </ul>
              </div>
            </section>
