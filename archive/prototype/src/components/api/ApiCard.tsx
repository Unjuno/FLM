...
const ApiCardComponent: React.FC<ApiCardProps> = ({
  ...
  // データベースでは「running」だが、プロキシサーバーが実際に起動していない場合
  const isProxyNotRunning = isRunning && api.proxy_running === false;
  ...
  return (
    <div className="api-card">
      <div className="api-card-header">
        ...
        <div
          className={`status-badge status-${api.status} ${isOperating ? 'operating' : ''} ${isProxyNotRunning ? 'proxy-not-running' : ''}`}
        >
          {isOperating ? (
            ...
            <span className="status-proxy-not-running-text">
              <span className="loading-spinner-small"></span>
              {t('apiList.status.proxyStarting') || 'プロキシ起動中...'}
            </span>
          ) : (
      ...
      <div className="api-info">
        ...
        {api.external_endpoint && (
          <div className="info-row">
            <span className="info-label">
              <Tooltip
                content="同一ネットワーク内の他のデバイスからこのAPIにアクセスする際に使用するURLです。"
                title="外部アクセスアドレス"
              >
                <span className="tooltip-label-with-icon">
                  外部アクセス
            ...
            <code className="info-value">{api.external_endpoint}</code>
          </div>
        )}
        <div className="info-row">
          <span className="info-label">
      ...
      <div className="api-actions">
        ...
        >
          ...
          >
            {isOperating ? (
              ...
              t('apiList.actions.stop')
            ) : (
              t('apiList.actions.start')
            )}
          </button>
        </Tooltip>
        ...
        <Tooltip
          content={t('apiList.actions.editTooltip') || 'API設定を変更'}
          position="top"
        >
          <button
