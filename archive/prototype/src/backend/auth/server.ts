...
import { rateLimitMiddleware, cleanupRateLimit } from './rate-limit.js';
import {
...
// 標準出力/標準エラー出力のバッファリングを無効化してログを即座に出力
// Tauriアプリケーションのコンソールにログが表示されるようにする
if (typeof process.stdout !== 'undefined' && process.stdout.isTTY) {
  // TTY（端末）に接続されている場合のみバッファリングを無効化
  process.stdout.setDefaultEncoding('utf8');
}
if (typeof process.stderr !== 'undefined' && process.stderr.isTTY) {
...
async function getPerformanceMetricsEnabledSetting(): Promise<boolean> {
  ...
  if (
    ...
    now - performanceMetricsEnabledCache.timestamp <
      SERVER_CONFIG.SETTING_CACHE_TTL
  ) {
...
async function collectPerformanceMetrics(
  ...
  try {
    ...
    if (globalCpuUsageState) {
      ...
      const deltaUser =
        currentCpuUsage.user - globalCpuUsageState.lastCpuUsage.user;
      const deltaSystem =
        currentCpuUsage.system - globalCpuUsageState.lastCpuUsage.system;
      const deltaTotal =
...
function startServers(certPaths: { certPath: string; keyPath: string }) {
  ...
  httpServer
    .listen(PORT, '0.0.0.0', () => {
      ...
      const bindAddress =
        address && typeof address !== 'string' ? address.address : '0.0.0.0';
      ...
      if (isExternallyAccessible) {
        console.log('🌐 [外部公開状態] HTTPサーバーは外部からアクセス可能です');
        console.log(
          `   バインドアドレス: ${bindAddress} (すべてのネットワークインターフェース)`
        );
        console.log(`   確認方法:`);
        console.log(
          `   - PowerShell: Get-NetTCPConnection -LocalPort ${PORT} -State Listen | Select-Object LocalAddress,LocalPort,State`
        );
        console.log(`   - コマンドプロンプト: netstat -an | findstr :${PORT}`);
        console.warn(
          '⚠️  [セキュリティ警告] HTTPサーバーは外部からアクセス可能です'
        );
        console.warn('   すべてのHTTPリクエストはHTTPSにリダイレクトされます');
      } else {
        console.log(
          '🔒 [外部公開状態] HTTPサーバーはローカルのみアクセス可能です'
        );
        console.log(`   バインドアドレス: ${bindAddress}`);
      }
    })
    .on('error', (err: NodeJS.ErrnoException) => {
      // エラーメッセージを先に出力（警告の前に）
      if (err.code === 'EADDRINUSE') {
        console.error(`[ERROR] ポート ${PORT} は既に使用されています。`);
        console.error(
          `[ERROR] 別のポートを指定するか、使用中のプロセスを終了してください。`
        );
        console.error(`[ERROR] エラーコード: ${err.code}`);
      } else {
        console.error(
          `[ERROR] HTTPリダイレクトサーバーの起動に失敗しました: ${err.message}`
        );
        console.error(`[ERROR] エラーコード: ${err.code || 'UNKNOWN'}`);
      }
      // エラーが発生した場合、プロセスを終了
      process.exit(1);
    });
  ...
  try {
    ...
    httpsServer
      .listen(httpsPort, '0.0.0.0', () => {
        ...
        console.log('');
        if (isExternallyAccessible) {
          console.log('🌐 [外部公開状態] サーバーは外部からアクセス可能です');
          console.log(
            `   バインドアドレス: ${bindAddress} (すべてのネットワークインターフェース)`
          );
          console.log(`   確認方法:`);
          console.log(
            `   - PowerShell: Get-NetTCPConnection -LocalPort ${httpsPort} -State Listen | Select-Object LocalAddress,LocalPort,State`
          );
          console.log(
            `   - コマンドプロンプト: netstat -an | findstr :${httpsPort}`
          );
          console.log(
            `   - 外部からアクセス: https://<このPCのIPアドレス>:${httpsPort}`
          );
        } else {
          console.log(
            '🔒 [外部公開状態] サーバーはローカルのみアクセス可能です'
          );
          console.log(`   バインドアドレス: ${bindAddress}`);
        }
        ...
        if (isExternallyAccessible) {
          console.warn('');
          console.warn(
            '⚠️  [セキュリティ警告] このサーバーは外部からアクセス可能です'
          );
          console.warn('   以下のセキュリティ対策を実施してください:');
          console.warn('   1. ファイアウォールで必要なIPアドレスのみ許可');
          console.warn(
            '   2. IPホワイトリスト機能の使用（ENABLE_IP_WHITELIST=1, IP_WHITELIST=xxx.xxx.xxx.xxx）'
          );
          console.warn('   3. APIキー認証が有効になっていることを確認');
          console.warn(
            "   4. 本番環境ではHTTPS（Let's Encrypt証明書）の使用を推奨"
          );
          console.warn('   5. 定期的なセキュリティアップデートの実施');
          console.warn(
            '   6. 不要な場合は、localhostのみにバインドすることを検討'
          );
        }
        ...
        if (isIpWhitelistEnabled()) {
          ...
          if (whitelist) {
            ...
            console.warn(
              '   ⚠️  警告: IPホワイトリストが有効ですが、ホワイトリストが空です'
            );
            console.warn(
              '   すべてのアクセスが拒否されます。IP_WHITELIST環境変数を設定してください。'
            );
          }
        } else {
          console.warn('');
          ...
          console.warn(
            '   外部公開する場合は、ENABLE_IP_WHITELIST=1 を設定してIPホワイトリストを有効化してください'
          );
        }
        console.warn('');
        ...
        if (!AUTH_REQUIRED) {
          console.error('');
          console.error(
            '🚨 [重大なセキュリティ警告] 認証が無効化されています！'
          );
          console.error(
            '   外部公開する場合は、必ず認証を有効にしてください。'
          );
      ...
      .on('error', (err: NodeJS.ErrnoException) => {
        // エラーメッセージを先に出力（警告の前に）
        if (err.code === 'EADDRINUSE') {
          console.error(`[ERROR] ポート ${httpsPort} は既に使用されています。`);
          console.error(
            `[ERROR] 別のポートを指定するか、使用中のプロセスを終了してください。`
          );
          console.error(`[ERROR] エラーコード: ${err.code}`);
          ...
          console.error(
            `[ERROR] HTTPSサーバーの起動に失敗しました: ${err.message}`
          );
          console.error(`[ERROR] エラーコード: ${err.code || 'UNKNOWN'}`);
        }
        // エラーが発生した場合、プロセスを終了
  ...
  // 未処理の例外をキャッチしてプロキシが停止しないようにする
  process.on('uncaughtException', (err: Error) => {
    console.error('[UNCAUGHT_EXCEPTION] 未処理の例外が発生しました:', err);
    if (err.stack) {
      console.error('[UNCAUGHT_EXCEPTION] スタックトレース:', err.stack);
    }
    // プロセスを終了させずに、エラーログを記録して続行
    // ただし、証明書エラーや起動エラーなどの致命的なエラーの場合は終了
    if (
      err.message.includes('certificate') ||
      err.message.includes('証明書') ||
      err.message.includes('EADDRINUSE') ||
      err.message.includes('ポート')
    ) {
      console.error(
        '[UNCAUGHT_EXCEPTION] 致命的なエラーのため、プロセスを終了します'
      );
      process.exit(1);
    }
    // その他のエラーはログに記録して続行
    console.warn(
      '[UNCAUGHT_EXCEPTION] エラーを記録しましたが、プロキシは継続実行します'
    );
  });
  ...
  process.on(
    ...
    (reason: unknown, promise: Promise<unknown>) => {
      console.error(
        '[UNHANDLED_REJECTION] 未処理のPromise拒否が発生しました:',
        reason
      );
      if (reason instanceof Error && reason.stack) {
        console.error('[UNHANDLED_REJECTION] スタックトレース:', reason.stack);
      }
      // プロセスを終了させずに、エラーログを記録して続行
      console.warn(
        '[UNHANDLED_REJECTION] エラーを記録しましたが、プロキシは継続実行します'
      );
    }
  );
}
...
/**
 * サーバーの状態情報を取得
 * @returns サーバーの状態情報（アドレス、ポート、外部公開状態など）
 */
export function getServerStatus(): {
  ...
  const getAddressInfo = (
    ...
    // TCPアドレスの場合
    const bindAddress = address.address;
  ...
  const httpInfo = getAddressInfo(httpAddress);
  const httpsInfo = getAddressInfo(httpsAddress);
  ...
  return {
    ...
    https: {
      isRunning: httpsServer !== null && httpsServer.listening,
      ...httpsInfo,
    },
  };
}
...
/**
 * サーバーが外部からアクセス可能かどうかを確認
 * @returns 外部からアクセス可能な場合はtrue
 */
export function isServerExternallyAccessible(): boolean {
  const status = getServerStatus();
  return (
    status.http.isExternallyAccessible || status.https.isExternallyAccessible
  );
}
