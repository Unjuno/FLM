...
pub fn validate_api_name(name: &str) -> Result<(), AppError> {
    ...
    // 空チェック
    if trimmed.is_empty() {
        return Err(AppError::validation_error("API名は空にできません"));
    }

    // 長さチェック
    if trimmed.len() < API_NAME_MIN_LENGTH {
        return Err(AppError::validation_error(format!(
            "API名は{}文字以上である必要があります",
            API_NAME_MIN_LENGTH
        )));
    }
...
pub fn validate_model_name(name: &str) -> Result<(), AppError> {
    ...
    // 空チェック
    if trimmed.is_empty() {
        return Err(AppError::validation_error("モデル名は空にできません"));
    }

    // 長さチェック
    if trimmed.len() < MODEL_NAME_MIN_LENGTH {
        return Err(AppError::validation_error(format!(
            "モデル名は{}文字以上である必要があります",
...
pub fn validate_ip_address(ip: &str) -> Result<(), AppError> {
    ...
    if trimmed.is_empty() {
        return Err(AppError::validation_error("IPアドレスは空にできません"));
    }
    ...
    if trimmed.contains('/') {
        ...
        parts[0]
            ...
            .map_err(|_| AppError::validation_error(format!("無効なIPアドレス: {}", parts[0])))?;

        // プレフィックス長の検証
        let prefix_len: u8 = parts[1].parse().map_err(|_| {
            AppError::validation_error(format!("無効なプレフィックス長: {}", parts[1]))
        })?;
        ...
        trimmed
            .parse::<std::net::IpAddr>()
            .map_err(|_| AppError::validation_error(format!("無効なIPアドレス: {}", trimmed)))?;
    }
