...
use crate::database::repository::security_repository::{
    ApiSecuritySettingsRepository, RateLimitTrackingRepository,
};
    ...
    let result = tokio::task::spawn_blocking(move || {
        ...
        if let Some(settings) = security_settings {
            ...
            if let Some(mut track) = tracking {
                ...
                RateLimitTrackingRepository::update(&conn, &track).map_err(|e| {
                ...
                RateLimitTrackingRepository::create(&conn, &api_id, &identifier, &window_start_iso)
                    .map_err(|e| AppError::DatabaseError {
