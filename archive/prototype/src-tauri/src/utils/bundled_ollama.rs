...
pub fn get_bundled_ollama_path() -> Result<Option<PathBuf>, AppError> {
    ...
    #[cfg(not(target_os = "windows"))]
    {
        ...
        if ollama_path.exists() {
            // 実行権限を確認
            let metadata = fs::metadata(&ollama_path).map_err(|e| {
                AppError::io_error(format!("Ollamaバイナリのメタデータ取得エラー: {}", e))
            })?;
            ...
            #[cfg(unix)]
            {
                ...
                fs::set_permissions(&ollama_path, perms).map_err(|e| {
                    AppError::io_error(format!("Ollamaバイナリの実行権限設定エラー: {}", e))
                })?;
            }
