...
fn get_key_from_keyring() -> Result<Option<[u8; 32]>, AppError> {
    ...
    let entry = Entry::new(KEYRING_SERVICE, KEYRING_USERNAME).map_err(|e| AppError::IoError {
...
fn get_encryption_key() -> Result<[u8; 32], AppError> {
    ...
    let key_bytes = if key_path.exists() {
        ...
        #[allow(deprecated)]
        // aes-gcm 0.10では非推奨だが、依存関係のバージョンアップは中長期改善項目
...
pub fn decrypt_api_key(encrypted: &str) -> Result<String, AppError> {
    ...
    // Nonceを12バイトの配列から作成
    let nonce_bytes: [u8; 12] = combined[..12].try_into().map_err(|_| AppError::IoError {
