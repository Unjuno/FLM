...
impl<'a> ApiRepository<'a> {
    ...
    pub fn find_by_id(&self, id: &str) -> Result<Api, DatabaseError> {
        let mut stmt = self.conn.prepare(
            "SELECT id, name, model, port, enable_auth, status, engine_type, engine_config, timeout_secs, created_at, updated_at FROM apis WHERE id = ?1"
        )?;
    ...
    pub fn find_all(&self) -> Result<Vec<Api>, DatabaseError> {
        let mut stmt = self.conn.prepare(
            "SELECT id, name, model, port, enable_auth, status, engine_type, engine_config, timeout_secs, created_at, updated_at FROM apis ORDER BY created_at DESC"
        )?;
    ...
    pub fn update(&self, api: &Api) -> Result<(), DatabaseError> {
        ...
        let rows_affected = self.conn.execute(
            r#"
            UPDATE apis 
            SET name = ?2, model = ?3, port = ?4, enable_auth = ?5, status = ?6, engine_type = ?7, engine_config = ?8, timeout_secs = ?9, updated_at = ?10
            WHERE id = ?1
            "#,
            params![
                api.id,
                ...
                engine_type,
                api.engine_config,
                api.timeout_secs.map(|v| v as i64),
                api.updated_at.to_rfc3339(),
            ],
        )?;
    ...
    pub fn find_by_name(&self, name: &str) -> Result<Option<Api>, DatabaseError> {
        let mut stmt = self.conn.prepare(
            "SELECT id, name, model, port, enable_auth, status, engine_type, engine_config, timeout_secs, created_at, updated_at FROM apis WHERE name = ?1"
        )?;
    ...
    pub fn find_by_port(&self, port: i32) -> Result<Option<Api>, DatabaseError> {
        let mut stmt = self.conn.prepare(
            "SELECT id, name, model, port, enable_auth, status, engine_type, engine_config, timeout_secs, created_at, updated_at FROM apis WHERE port = ?1"
        )?;
    ...
    fn row_to_api(row: &Row) -> Result<Api, rusqlite::Error> {
        ...
        let timeout_secs: Option<u64> = row
            .get::<_, Option<i64>>(8)
            .ok()
            .flatten()
            .map(|v| v as u64);
...
impl<'a> RequestLogRepository<'a> {
    ...
    pub fn find_with_filters(
        ...
        // API IDフィルタ
        if api_id.is_some() {
            conditions.push(format!("api_id = ?{}", next_param_idx));
            next_param_idx += 1;
        }

        // 開始日時フィルタ
        if start_date.is_some() {
            conditions.push(format!("created_at >= ?{}", next_param_idx));
            next_param_idx += 1;
        }
        ...
        if let Some(status_codes_val) = status_codes {
            if !status_codes_val.is_empty() {
                ...
                next_param_idx += status_codes_val.len();
            }
        }
        ...
        if let Some(path_filter_val) = path_filter {
            if !path_filter_val.is_empty() {
                conditions.push(format!("path LIKE ?{}", next_param_idx));
                next_param_idx += 1;
            }
        }
        ...
        let limit_param_idx = next_param_idx;
        let offset_param_idx = next_param_idx + 1;
        // 次のクエリ構築で使用される可能性があるため、next_param_idxを更新
