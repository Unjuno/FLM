...
pub fn create_schema(conn: &Connection) -> Result<(), DatabaseError> {
    ...
    conn.execute(
        ...
        CREATE TABLE IF NOT EXISTS apis (
            ...
            CHECK (timeout_secs IS NULL OR (timeout_secs > 0 AND timeout_secs <= 600)),
            UNIQUE(port)
        )
        "#,
        [],
    ...
    // 既存のテーブルにtimeout_secsカラムを追加（マイグレーション）
    let _ = conn.execute("ALTER TABLE apis ADD COLUMN timeout_secs INTEGER", []);

    // 既存のテーブルにdefault_api_timeout_secs設定を追加（マイグレーション）
    let now = chrono::Utc::now().to_rfc3339();
    let _ = conn.execute(
        r#"
        INSERT OR IGNORE INTO user_settings (key, value, updated_at) VALUES
            ('default_api_timeout_secs', '30', ?1)
        "#,
        params![now],
    );
