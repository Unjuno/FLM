...
impl ApiRepository {
    ...
    pub fn find_all(conn: &Connection) -> Result<Vec<Api>, DatabaseError> {
        ...
        let mut stmt = conn.prepare(
            "SELECT id, name, model, port, enable_auth, status, 
             COALESCE(engine_type, 'ollama'), engine_config, timeout_secs, created_at, updated_at 
        ...
        let api_iter = stmt.query_map([], |row| {
            Ok(Api {
                ...
                timeout_secs: row.get::<_, Option<i64>>(8)?.map(|v| v as u64),
                created_at: DateTime::parse_from_rfc3339(&row.get::<_, String>(9)?)
                    .map_err(|_| {
                        RusqliteError::InvalidColumnType(9, "created_at".to_string(), Type::Text)
                    })?
                    .with_timezone(&Utc),
                updated_at: DateTime::parse_from_rfc3339(&row.get::<_, String>(10)?)
                    .map_err(|_| {
                        RusqliteError::InvalidColumnType(10, "updated_at".to_string(), Type::Text)
                    })?
                    .with_timezone(&Utc),
            })
        })?;
    ...
    pub fn create(conn: &Connection, api: &Api) -> Result<(), DatabaseError> {
        ...
        conn.execute(
            "INSERT INTO apis (id, name, model, port, enable_auth, status, engine_type, engine_config, timeout_secs, created_at, updated_at) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            params![
                api.id,
                api.name,
    ...
    pub fn update(conn: &Connection, api: &Api) -> Result<(), DatabaseError> {
        ...
        conn.execute(
            "UPDATE apis SET name = ?, model = ?, port = ?, enable_auth = ?, status = ?, engine_type = ?, engine_config = ?, timeout_secs = ?, updated_at = ? WHERE id = ?",
            params![
                api.name,
                api.model,
                api.port,
