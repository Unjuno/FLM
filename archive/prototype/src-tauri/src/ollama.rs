...
pub async fn stop_ollama() -> Result<(), AppError> {
    eprintln!("[DEBUG] stop_ollama 開始");
    // まず、Ollamaが実行中かどうかを確認
    let was_running = check_ollama_running().await.unwrap_or(false);
    eprintln!("[DEBUG] stop_ollama: was_running={}", was_running);
    if !was_running {
        // 既に停止している場合は成功として返す
        eprintln!("[DEBUG] stop_ollama: 既に停止しているため成功として返す");
        return Ok(());
    }
    ...
    eprintln!("[DEBUG] stop_ollama: 停止APIを呼び出し: url={}", url);
    let response = client
        .delete(&url)
        .send()
        .await
        .map_err(|e| {
            eprintln!("[ERROR] stop_ollama: API呼び出しエラー: {}", e);
    ...
    eprintln!("[DEBUG] stop_ollama: APIレスポンス status={}", status);

    // レスポンスのステータスコードをチェック
    // 200 OK または 204 No Content が正常なレスポンス
    if !status.is_success() && status != 204 {
        eprintln!("[ERROR] stop_ollama: APIエラー status={}", status);
        return Err(AppError::OllamaError {
            message: format!("Ollama停止APIエラー: HTTP {}", status),
            source_detail: None,
        });
    }

    eprintln!("[DEBUG] stop_ollama: API呼び出し成功、停止確認を開始");
    // 停止後に実際に停止したかを確認（最大5秒間、0.5秒間隔で確認）
    for i in 0..10 {
        tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;
        let still_running = check_ollama_running().await.unwrap_or(false);
        eprintln!("[DEBUG] stop_ollama: 停止確認試行 {}/10, still_running={}", i + 1, still_running);
        if !still_running {
            // 停止が確認できた
            eprintln!("[DEBUG] stop_ollama: 停止が確認できた");
            return Ok(());
        }
    }

    // 5秒経過してもまだ実行中の場合は警告を出すが、エラーにはしない
    // （OllamaのシャットダウンAPIは呼び出し成功しているため）
    eprintln!("[WARN] Ollama停止APIは呼び出し成功しましたが、5秒経過後もまだ実行中の可能性があります");
    Ok(())
}
