...
use crate::ollama::{current_ollama_host_port, OllamaDetectionResult};
...
pub async fn detect_ollama() -> Result<OllamaStatus, AppError> {
    ...
    let result = match crate::ollama::detect_ollama().await {
        Ok(r) => {
...
pub async fn download_ollama(
    ...
    let path = crate::ollama::download_ollama(move |progress| {
        // 進捗をイベントとして送信
...
pub async fn start_ollama(ollama_path: Option<String>) -> Result<u32, AppError> {
    ...
    let pid = match crate::ollama::start_ollama(ollama_path).await {
        Ok(pid) => {
...
pub async fn stop_ollama() -> Result<(), AppError> {
    crate::ollama::stop_ollama().await?;
    Ok(())
}
...
pub async fn check_ollama_health() -> Result<OllamaHealthStatus, AppError> {
    let running = crate::ollama::check_ollama_running().await.unwrap_or_else(|e| {
...
pub async fn check_ollama_update() -> Result<OllamaUpdateCheck, AppError> {
    let (is_newer_available, current_version, latest_version) =
        crate::ollama::check_ollama_update_available().await?;
...
pub async fn update_ollama(app: AppHandle) -> Result<String, AppError> {
    // 1. 既存のOllamaを停止
    let was_running = crate::ollama::check_ollama_running().await.unwrap_or_else(|e| {
        eprintln!("[WARN] Ollamaの実行状態確認に失敗しました: {}", e);
        false
    });
    if was_running {
    ...
    // 2. 最新版をダウンロード
    let app_handle = app.clone();
    let new_path = crate::ollama::download_ollama(move |progress| {
        // 進捗をイベントとして送信
    ...
    if was_running {
        ...
        crate::ollama::start_ollama(Some(new_path.clone())).await?;
    }
