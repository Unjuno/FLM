...
use crate::engines::installer::{install_llama_cpp, install_lm_studio, install_vllm};
use crate::engines::updater::{
    check_llama_cpp_update, check_lm_studio_update, check_vllm_update, update_llama_cpp,
    update_lm_studio, update_vllm,
};
use crate::engines::{
    EngineConfig, EngineConfigData, EngineDetectionResult, EngineDownloadProgress, EngineManager,
    EngineUpdateCheck,
};
use chrono::Utc;
use rusqlite::params;
...
pub async fn detect_engine(engineType: String) -> Result<EngineDetectionResult, String> {
    let engine_type = engineType.clone();
    eprintln!(
        "[DEBUG] detect_engineコマンド呼び出し: engine_type={}",
        engine_type
    );
    let manager = get_engine_manager();
    match manager.detect_engine(&engine_type).await {
        Ok(result) => {
            eprintln!(
                "[DEBUG] detect_engine成功: installed={}, running={}, message={:?}",
        ...
        Err(e) => {
            eprintln!(
                "[ERROR] detect_engine失敗: engine_type={}, error={}",
                engine_type, e
            );
...
#[tauri::command]
#[allow(non_snake_case)] // Tauri v2のフロントエンドとの互換性のためcamelCaseを使用
pub async fn start_engine(engineType: String, config: Option<EngineConfig>) -> Result<u32, String> {
    let engine_type = engineType;
    let manager = get_engine_manager();
    manager
        .start_engine(&engine_type, config)
        .await
        .map_err(|e| format!("{}", e))
}
...
#[tauri::command]
#[allow(non_snake_case)] // Tauri v2のフロントエンドとの互換性のためcamelCaseを使用
pub async fn stop_engine(engineType: String) -> Result<(), String> {
    let engine_type = engineType;
    eprintln!("[DEBUG] stop_engineコマンド呼び出し: engine_type={}", engine_type);
    let manager = get_engine_manager();
    match manager.stop_engine(&engine_type).await {
        Ok(_) => {
            eprintln!("[DEBUG] stop_engine成功: engine_type={}", engine_type);
            Ok(())
        }
        Err(e) => {
            eprintln!("[ERROR] stop_engine失敗: engine_type={}, error={}", engine_type, e);
            Err(format!("{}", e))
        }
    }
}
...
/// エンジン設定一覧を取得
#[tauri::command]
pub async fn get_engine_configs(
    engine_type: Option<String>,
) -> Result<Vec<EngineConfigData>, String> {
    let conn = get_connection().map_err(|e| format!("データベース接続エラー: {}", e))?;
...
pub async fn get_engine_models(
    engineType: String,
) -> Result<Vec<crate::engines::ModelInfo>, String> {
    let engine_type = engineType;
    let manager = get_engine_manager();
    manager
        .get_engine_models(&engine_type)
        .await
        .map_err(|e| format!("{}", e))
}
...
#[tauri::command]
#[allow(non_snake_case)] // Tauri v2のフロントエンドとの互換性のためcamelCaseを使用
pub async fn install_engine(app_handle: AppHandle, engineType: String) -> Result<String, String> {
    let engine_type = engineType;
    ...
    let progress_callback =
        |progress: EngineDownloadProgress| -> Result<(), crate::utils::error::AppError> {
            if let Err(e) = app_handle.emit("engine_install_progress", &progress) {
                eprintln!(
                    "[WARN] エンジンインストール進捗イベントの送信に失敗しました: {}",
                    e
                );
            }
            Ok(())
    ...
    match engine_type.as_str() {
        "lm_studio" => install_lm_studio(progress_callback)
            .await
            .map_err(|e| format!("LM Studioのインストールに失敗しました: {}", e)),
        "vllm" => install_vllm(progress_callback)
            .await
            .map_err(|e| format!("vLLMのインストールに失敗しました: {}", e)),
        "llama_cpp" => install_llama_cpp(progress_callback)
            .await
            .map_err(|e| format!("llama.cppのインストールに失敗しました: {}", e)),
        "ollama" => {
            // Ollamaは別のコマンド（download_ollama）を使用
            Err(
                "Ollamaのインストールには「Ollamaセットアップ」機能を使用してください。"
                    .to_string(),
            )
        }
        _ => Err(format!("不明なエンジンタイプ: {}", engine_type)),
    }
}
...
pub async fn check_engine_update(engine_type: String) -> Result<EngineUpdateCheck, String> {
    match engine_type.as_str() {
        "lm_studio" => check_lm_studio_update()
            .await
            .map_err(|e| format!("LM Studioのアップデート確認に失敗しました: {}", e)),
        "vllm" => check_vllm_update()
            .await
            .map_err(|e| format!("vLLMのアップデート確認に失敗しました: {}", e)),
        "llama_cpp" => check_llama_cpp_update()
            .await
...
/// エンジンをアップデート
#[tauri::command]
pub async fn update_engine(app_handle: AppHandle, engine_type: String) -> Result<String, String> {
    // 進捗をイベントで送信するコールバック
    let progress_callback =
        |progress: EngineDownloadProgress| -> Result<(), crate::utils::error::AppError> {
    ...
    match engine_type.as_str() {
        "lm_studio" => update_lm_studio(progress_callback)
            .await
            .map_err(|e| format!("LM Studioのアップデートに失敗しました: {}", e)),
        "vllm" => update_vllm(progress_callback)
            .await
            .map_err(|e| format!("vLLMのアップデートに失敗しました: {}", e)),
        "llama_cpp" => update_llama_cpp(progress_callback)
            .await
