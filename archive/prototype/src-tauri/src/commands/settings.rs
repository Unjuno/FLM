...
pub struct AppSettings {
    ...
    /// デフォルトAPIタイムアウト（秒、デフォルト: 30）
    pub default_api_timeout_secs: Option<u64>,
}
...
pub async fn get_app_settings() -> Result<AppSettings, String> {
    ...
    let language = settings_repo
        .get("language")
        .map_err(|e| format!("設定の読み込みに失敗しました: {}", e))?;

    let auto_refresh_interval = settings_repo
        .get("auto_refresh_interval")
        .map_err(|e| format!("設定の読み込みに失敗しました: {}", e))?
        .and_then(|v| v.parse::<u32>().ok());

    let log_retention_days = settings_repo
        .get("log_retention_days")
        .map_err(|e| format!("設定の読み込みに失敗しました: {}", e))?
        .and_then(|v| v.parse::<u32>().ok());

    let audit_log_retention_days = settings_repo
    ...
    let stop_apis_on_exit = settings_repo
        .get("stop_apis_on_exit")
        .map_err(|e| format!("設定の読み込みに失敗しました: {}", e))?
        .and_then(|v| v.parse::<bool>().ok());

    let save_request_body = settings_repo
        .get("save_request_body")
        .map_err(|e| format!("設定の読み込みに失敗しました: {}", e))?
        .and_then(|v| v.parse::<bool>().ok());

    let diagnostics_enabled = settings_repo
        .get("diagnostics_enabled")
        .map_err(|e| format!("設定の読み込みに失敗しました: {}", e))?
        .and_then(|v| v.parse::<bool>().ok());

    let performance_metrics_enabled = settings_repo
        .get("performance_metrics_enabled")
        .map_err(|e| format!("設定の読み込みに失敗しました: {}", e))?
        .and_then(|v| v.parse::<bool>().ok());

    let include_ip_address_in_audit_log = settings_repo
    ...
    let backup_encrypt_by_default = settings_repo
        .get("backup_encrypt_by_default")
        .map_err(|e| format!("設定の読み込みに失敗しました: {}", e))?
        .and_then(|v| v.parse::<bool>().ok());

    let show_incomplete_features = settings_repo
        .get("show_incomplete_features")
        .map_err(|e| format!("設定の読み込みに失敗しました: {}", e))?
        .and_then(|v| v.parse::<bool>().ok());
    ...
    let default_api_timeout_secs = settings_repo
        .get("default_api_timeout_secs")
        .map_err(|e| format!("設定の読み込みに失敗しました: {}", e))?
    ...
    Ok(AppSettings {
        ...
        default_api_timeout_secs: default_api_timeout_secs.or(Some(30)),    // デフォルト: 30秒
    })
}
...
pub async fn update_app_settings(settings: AppSettings) -> Result<(), String> {
    ...
    if let Some(theme) = settings.theme {
        settings_repo
            .set("theme", &theme)
            .map_err(|e| format!("テーマ設定の保存に失敗しました: {}", e))?;
    }

    if let Some(language) = settings.language {
        settings_repo
            .set("language", &language)
            .map_err(|e| format!("言語設定の保存に失敗しました: {}", e))?;
    }

    if let Some(interval) = settings.auto_refresh_interval {
        settings_repo
            .set("auto_refresh_interval", &interval.to_string())
            .map_err(|e| format!("自動更新間隔の保存に失敗しました: {}", e))?;
    }

    if let Some(days) = settings.log_retention_days {
        settings_repo
            .set("log_retention_days", &days.to_string())
            .map_err(|e| format!("ログ保持期間の保存に失敗しました: {}", e))?;
    }

    if let Some(days) = settings.audit_log_retention_days {
        settings_repo
            .set("audit_log_retention_days", &days.to_string())
            .map_err(|e| format!("監査ログ保持期間の保存に失敗しました: {}", e))?;
    }

    if let Some(enabled) = settings.notifications_enabled {
        settings_repo
            .set("notifications_enabled", &enabled.to_string())
            .map_err(|e| format!("通知設定の保存に失敗しました: {}", e))?;
    }

    if let Some(stop_on_exit) = settings.stop_apis_on_exit {
        settings_repo
            .set("stop_apis_on_exit", &stop_on_exit.to_string())
            .map_err(|e| format!("アプリ終了時のAPI停止設定の保存に失敗しました: {}", e))?;
    }

    if let Some(save_body) = settings.save_request_body {
        settings_repo
            .set("save_request_body", &save_body.to_string())
            .map_err(|e| format!("リクエストボディ保存設定の保存に失敗しました: {}", e))?;
    }

    if let Some(enabled) = settings.diagnostics_enabled {
        settings_repo
            .set("diagnostics_enabled", &enabled.to_string())
            .map_err(|e| format!("診断機能設定の保存に失敗しました: {}", e))?;
    }

    if let Some(enabled) = settings.performance_metrics_enabled {
        settings_repo
            .set("performance_metrics_enabled", &enabled.to_string())
            .map_err(|e| format!("パフォーマンスメトリクス設定の保存に失敗しました: {}", e))?;
    }

    if let Some(enabled) = settings.include_ip_address_in_audit_log {
        settings_repo
            .set("include_ip_address_in_audit_log", &enabled.to_string())
            .map_err(|e| format!("監査ログIPアドレス設定の保存に失敗しました: {}", e))?;
    }

    if let Some(enabled) = settings.device_id_enabled {
        settings_repo
            .set("device_id_enabled", &enabled.to_string())
            .map_err(|e| format!("デバイスID設定の保存に失敗しました: {}", e))?;
    }
    ...
    if let Some(timeout) = settings.default_api_timeout_secs {
        settings_repo
            .set("default_api_timeout_secs", &timeout.to_string())
            .map_err(|e| format!("デフォルトAPIタイムアウト設定の保存に失敗しました: {}", e))?;
    }
