...
pub async fn suggest_api_name(base_name: Option<String>) -> Result<ApiNameSuggestion, String> {
    ...
    let conn = get_connection().map_err(|e| {
        format!(
            "データベース接続エラー: {}. アプリを再起動して再度お試しください。",
            e
        )
    })?;
    ...
    debug_log!("API名の自動生成開始: ベース名=\"{}\"", base);

    // まず、元の名前が使用可能かチェック
    let is_available = match api_repo.find_by_name(&base) {
        Ok(::std::option::Option::None) => {
            debug_log!("ベース名 \"{}\" は使用可能です", base);
            true
        }
        Ok(Some(existing_api)) => {
            debug_log!(
                r#"ベース名 "{}" は既にAPI「{}」で使用されています"#,
                base,
                existing_api.name
            );
            false
        ...
        Err(e) => {
            debug_log!("ベース名 \"{}\" のチェック中にエラーが発生: {}", base, e);
            false
        }
    };
    ...
    if !is_available {
        ...
        loop {
            ...
            debug_log!("候補名をチェック: \"{}\"", candidate);

            match api_repo.find_by_name(&candidate) {
                Ok(::std::option::Option::None) => {
                    suggested_name = candidate.clone();
                    info_log!("推奨API名を生成: \"{}\"", suggested_name);
                    break;
                }
                Ok(Some(_)) => {
                    debug_log!("候補名 \"{}\" は既に使用されています", candidate);
                    counter += 1;
                    if counter <= 5 {
                ...
                Err(e) => {
                    debug_log!("候補名 \"{}\" のチェック中にエラーが発生: {}", candidate, e);
            ...
            if counter > 100 {
                ...
                suggested_name = format!("{} ({})", base, timestamp);
                info_log!(
                    "100回の試行後、タイムスタンプ付きの名前を生成: \"{}\"",
                    suggested_name
                );
                break;
            }
        ...
        info_log!("ベース名 \"{}\" は使用可能です", base);
    }

    // 代替名を生成（モデル名ベース、日時ベースなど）
    if alternatives.len() < 5 {
        debug_log!("代替名を生成中（現在: {}個）", alternatives.len());

        let timestamp = chrono::Utc::now().format("%Y%m%d_%H%M%S").to_string();
        let timestamp_name = format!("API {}", timestamp);
        if let Ok(::std::option::Option::None) = api_repo.find_by_name(&timestamp_name) {
            alternatives.push(timestamp_name);
        }

        let date = chrono::Utc::now().format("%Y-%m-%d").to_string();
        let date_name = format!("API {}", date);
        if let Ok(::std::option::Option::None) = api_repo.find_by_name(&date_name) {
            alternatives.push(date_name);
        }
    ...
    info_log!(
        "API名の自動生成完了: 推奨名=\"{}\", 代替名={}個, 元の名前は使用可能={}",
        suggested_name,
        alternatives.len(),
        is_available
    );
