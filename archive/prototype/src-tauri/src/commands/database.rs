...
pub async fn rollback_database_migration(target_version: i32) -> Result<String, String> {
    let mut conn = get_connection().map_err(|e| format!("データベース接続エラー: {}", e))?;

    rollback_migration(&mut conn, target_version)
        .map_err(|e| format!("マイグレーションのロールバックに失敗しました: {}", e))?;

    Ok(format!(
        "マイグレーションをバージョン{}までロールバックしました",
        target_version
    ))
}
