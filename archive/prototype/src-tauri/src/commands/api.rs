...
use crate::database::repository::{
    error_log_repository::ErrorLogRepository, ApiKeyRepository, ApiRepository,
    InstalledModelRepository, ModelCatalogRepository, RequestLogRepository,
};
...
pub struct ApiCreateConfig {
    ...
    pub timeout_secs: Option<u64>,   // タイムアウト（秒、Noneの場合はグローバル設定を使用）
}
...
pub struct ApiInfo {
    ...
    pub name: String,
    pub endpoint: String,
    pub external_endpoint: Option<String>, // 外部アクセス用エンドポイント（ローカルIPアドレス）
    pub model_name: String,
    pub port: u16,
...
pub async fn create_api(config: ApiCreateConfig) -> Result<ApiCreateResponse, String> {
    ...
    // データベースに見つからない場合、エンジンから直接確認
    let model_exists = if !model_exists_in_db {
        let manager = EngineManager::new();
        
        // エンジンが起動しているか確認
        let is_running = manager.is_engine_running(engine_type).await.unwrap_or(false);
        
        // エンジンが起動していない場合、自動的に起動を試みる
        if !is_running {
            info_log!("エンジン「{}」が起動していないため、自動起動を試みます", engine_type);
            match manager.start_engine(engine_type, None).await {
                Ok(_) => {
                    info_log!("エンジン「{}」の起動に成功しました", engine_type);
                    // エンジンが起動するまで少し待機（最大10秒）
                    let mut waited = 0;
                    while waited < 10 {
                        tokio::time::sleep(tokio::time::Duration::from_secs(1)).await;
                        if manager.is_engine_running(engine_type).await.unwrap_or(false) {
                            info_log!("エンジン「{}」が起動しました", engine_type);
                            break;
                        }
                        waited += 1;
                    }
                ...
                Err(e) => {
                    ...
                    // 起動に失敗しても、既に起動している可能性があるので続行
                }
            }
        }
        
        // エンジンからモデル一覧を取得
        match manager.get_engine_models(engine_type).await {
            Ok(models) => {
                let found = models.iter().any(|m| m.name == model_name);
                
                // エンジンに存在する場合は、データベースに登録
                if found {
                    if let Some(model_info) = models.iter().find(|m| m.name == model_name) {
                        let installed_model = InstalledModel {
                            name: model_name.clone(),
                            size: model_info.size.unwrap_or(0) as i64,
                            parameters: model_info.parameter_size.as_ref()
                                .and_then(|ps| {
                                    ps.trim_end_matches('B').parse::<f64>().ok()
                                        .map(|num| (num * 1_000_000_000.0) as i64)
                                }),
                        ...
                        let _ = tokio::task::spawn_blocking({
                            let model = installed_model.clone();
                            move || {
                        ...
                        info_log!("モデル「{}」をエンジン「{}」から検出し、データベースに登録しました", model_name, engine_type);
                    }
                }
            ...
            Err(e) => {
                warn_log!("エンジン「{}」からモデル一覧を取得できませんでした: {}", engine_type, e);
                false
            }
        }
    } else {
    ...
    if let Some(ref engine_config_json) = config.engine_config {
        ...
        model_params::validate_engine_config(engine_config_json.as_str()).map_err(|e| {
            format!(
                "エンジン設定の検証に失敗しました（API名: {name}、エンジン: {engine}）: {e}",
                name = config.name,
                engine = engine_type
            )
        })?;
    ...
    if port < 1024 || port > 65535 {
        return Err(format!(
            "無効なポート番号: {}。ポート番号は1024から65535の間である必要があります。",
            port
        ));
    }
...
pub async fn list_apis() -> Result<Vec<ApiInfo>, String> {
    let conn = get_connection().map_err(|e| {
        format!(
            "データベース接続に失敗しました: {}. アプリを再起動して再度お試しください。",
            e
        )
    })?;
    ...
    let apis = api_repo.find_all().map_err(|e| {
        format!(
            "作成済みAPIの一覧を取得できませんでした: {}. アプリを再起動して再度お試しください。",
            e
        )
    })?;
    ...
    let api_infos: Vec<ApiInfo> = apis
        ...
        .map(|api| {
            ...
            let external_endpoint = network::format_external_endpoint_url(api.port as u16, Some(&api.id));
            ApiInfo {
                id: api.id,
                name: api.name,
                endpoint,
        ...
        move || {
            ...
            let engine_base_url = api
                ...
                .map(|et| match et {
                    ...
                    "vllm" => {
                        use crate::engines::{traits::LLMEngine, vllm::VLLMEngine};
    ...
    let was_running = tokio::task::spawn_blocking({
        ...
        move || {
            ...
            match api_repo.find_by_id(&api_id) {
                Ok(api) => {
                    Ok::<(bool, u16), String>((api.status == ApiStatus::Running, api.port as u16))
                }
                Err(_) => Ok((false, 0)),
            }
        }
...
pub async fn get_api_details(apiId: String) -> Result<ApiDetailsResponse, String> {
    ...
    let api = api_repo
        .find_by_id(&api_id)
        .map_err(|e: DatabaseError| format!("API取得エラー: {e}"))?;
    ...
    let api_key = if api.enable_auth {
        ...
        match key_repo.find_by_api_id(&api_id) {
            Ok(Some(key_data)) => {
                ...
                encryption::decrypt_api_key(&encrypted_str)
                    .map_err(|e| {
...
pub async fn update_api(api_id: String, config: ApiUpdateConfig) -> Result<(), String> {
    ...
    if let Some(ref name) = config.name {
        input_validation::validate_api_name(name)
            .map_err(|e| format!("API名の検証に失敗しました: {}", e))?;
    }
    ...
    let (new_port, new_enable_auth, needs_restart_flag) = tokio::task::spawn_blocking({
        ...
        move || {
            ...
            if let Some(timeout_secs) = config.timeout_secs {
                api.timeout_secs = Some(timeout_secs);
                // タイムアウト設定の変更は再起動不要（HTTPリクエスト時に動的に適用される）
            }

            api.updated_at = Utc::now();

            api_repo.update(&api).map_err(|_| {
                "APIの設定を更新できませんでした。アプリを再起動して再度お試しください。"
                    .to_string()
            })?;
    ...
    if needs_restart_flag && was_running {
        ...
        let (engine_type_for_restart, engine_base_url_for_restart) = tokio::task::spawn_blocking({
            ...
            move || {
                let conn =
                    get_connection().map_err(|_| "データの読み込みに失敗しました".to_string())?;
                let api_repo = ApiRepository::new(&conn);
                let api = api_repo
                    .find_by_id(&api_id)
                    .map_err(|_| "APIが見つかりませんでした".to_string())?;
    ...
    match key_data {
        Some(key_data) => {
            ...
            let decrypted_key = encryption::decrypt_api_key(&encrypted_str).map_err(|_| {
                "セキュリティキーの復号化に失敗しました。アプリを再起動して再度お試しください。"
                    .to_string()
            })?;
...
pub async fn regenerate_api_key(apiId: String) -> Result<String, String> {
    ...
    let encrypted_key_str = encryption::encrypt_api_key(&generated_key)
        .map_err(|e| format!("APIキー暗号化エラー: {e}"))?;

    // Base64デコードしてバイト配列に変換（データベースに保存するため）
    let encrypted_key_bytes = STANDARD
        .decode(&encrypted_key_str)
        .map_err(|e| format!("暗号化データのデコードエラー: {e}"))?;
    ...
    match key_repo.find_by_api_id(&api_id) {
        Ok(Some(mut key_data)) => {
            ...
            key_repo
                .update(&key_data)
                .map_err(|e: DatabaseError| format!("APIキー更新エラー: {e}"))?;
        }
        ...
        Ok(None) | Err(_) => {
            ...
            key_repo.create(&api_key_data).map_err(|_| {
                "セキュリティキーの保存に失敗しました。アプリを再起動して再度お試しください。"
                    .to_string()
            })?;
        }
    }
...
pub async fn delete_api_key(apiId: String) -> Result<(), String> {
    ...
    // APIキーを削除
    key_repo
        .delete_by_api_id(&api_id)
        .map_err(|e: DatabaseError| format!("APIキー削除エラー: {e}"))?;
...
pub async fn get_models_list() -> Result<Vec<ModelInfo>, String> {
    ...
    if !check_ollama_running()
        .await
        .map_err(|_| "AIエンジンの状態を確認できませんでした。".to_string())?
    {
        return Err(
            "AIエンジンが実行されていません。先にAIエンジンを起動してください。".to_string(),
        );
    }
    ...
    let response = client.get(&tags_endpoint).send().await.map_err(|_| {
        "AIエンジンに接続できませんでした。AIエンジンが正常に起動しているか確認してください。"
            .to_string()
    })?;

    let tags: serde_json::Value = response.json().await.map_err(|_| {
        "AIエンジンからの応答を読み取れませんでした。AIエンジンを再起動して再度お試しください。"
            .to_string()
    })?;
...
fn extract_parameter_size(model_name: &str) -> Option<String> {
    ...
    let re = regex::Regex::new(r"(\d+(?:\.\d+)?)\s*[bB]")
        .map_err(|e| {
            warn_log!(
                " パラメータサイズ抽出用の正規表現コンパイルに失敗しました: {}",
                e
            );
            e
        })
...
/// 進捗はイベント経由で送信されます
#[tauri::command]
pub async fn download_model(model_name: String, app: tauri::AppHandle) -> Result<(), String> {
    ...
    if !status.is_success() {
        ...
        let error_body = response
            .text()
            .await
            .unwrap_or_else(|_| "レスポンス本文を読み取れませんでした".to_string());
            ...
            serde_json::from_str::<serde_json::Value>(&error_body)
        {
            if let Some(error_msg) = error_json.get("error").and_then(|e| e.as_str()) {
                ...
                format!(
                    "AIモデルのダウンロードに失敗しました（HTTP {}）。エラー詳細: {}",
                    status, error_body
                )
            }
        } else {
    ...
    let mut update_progress_stats = |json: &serde_json::Value| -> (u64, u64) {
        ...
        if let Some(ref digest_key) = digest_key_option {
            ...
            if let Some(completed) = json.get("completed").and_then(|c| c.as_u64()) {
                let total_for_key = layer_totals.get(digest_key).copied().unwrap_or_default();
        ...
        while let Some(newline_pos) = buffer.find('\n') {
            ...
            if !line_str.is_empty() {
                ...
                if let Ok(json) = serde_json::from_str::<serde_json::Value>(&line_str) {
                    ...
                    let (current_downloaded_bytes, current_total_bytes) =
                        update_progress_stats(&json);
        ...
        .map_err(|_| "AIエンジンの状態を確認できませんでした。".to_string())?
    {
        let client = match crate::utils::http_client::create_http_client() {
            ...
            Err(e) => {
                ...
                warn_log!(
                    " HTTPクライアントの作成に失敗しました。データベースの情報のみを返します: {}",
                    e
                );
                // データベースから取得したモデル情報を返す（Ollama APIからの追加情報なし）
                // InstalledModelInfo形式に変換
                let result: Vec<InstalledModelInfo> = installed_models
                    .into_iter()
                    .map(|model| InstalledModelInfo {
                        name: model.name,
                        ...
                        for ollama_model in ollama_models {
                            if let Some(name) = ollama_model["name"].as_str() {
                                if let Some(size) = ollama_model["size"].as_u64() {
                                    // メモリ上のデータを更新
                                    if let Some(db_model) =
                                        installed_models.iter_mut().find(|m| m.name == name)
                                    {
                                        ...
                                        let parameter_size =
                                            extract_parameter_size(name).and_then(|s| {
                                                if let Ok(num) =
                                                    s.trim_end_matches('B').parse::<f64>()
                                                {
...
pub async fn get_request_logs(
    request: GetRequestLogsRequest,
) -> Result<GetRequestLogsResponse, String> {
    let conn = get_connection().map_err(|_| {
        "データの読み込みに失敗しました。アプリを再起動して再度お試しください。".to_string()
    })?;
    ...
    let logs = log_repo
        ...
        .map_err(|e| format!("リクエストログの取得に失敗しました: {e}"))?;

    // 総件数を取得（CODE-002修正: 正確な総件数を取得）
    let total_count = log_repo
        .count_with_filters(
            request.api_id.as_deref(),
            request.start_date.as_deref(),
            request.end_date.as_deref(),
        ...
        .map_err(|e| format!("ログ総件数の取得に失敗しました: {e}"))?;

    let result: Vec<RequestLogInfo> = logs
        .into_iter()
        .map(|log| RequestLogInfo {
            id: log.id,
            api_id: log.api_id,
            method: log.method,
            path: log.path,
...
pub async fn get_log_statistics(request: GetLogStatisticsRequest) -> Result<LogStatistics, String> {
    ...
    let (total_requests, avg_response_time_ms, error_rate, status_code_distribution) = log_repo
        ...
        .map_err(|e| format!("ログ統計情報の取得に失敗しました: {e}"))?;
...
pub async fn export_logs(request: ExportLogsRequest) -> Result<ExportLogsResponse, String> {
    ...
    let data = match request.format.as_str() {
        ...
        "json" => {
            // JSON形式に変換
            let log_infos: Vec<RequestLogInfo> = logs
                .into_iter()
                .map(|log| RequestLogInfo {
                    id: log.id,
                    api_id: log.api_id,
            ...
            serde_json::to_string_pretty(&log_infos)
                .map_err(|e| format!("JSON変換に失敗しました: {e}"))?
        }
        _ => {
            return Err(
                "無効なフォーマットです。'csv' または 'json' を指定してください。".to_string(),
            );
        }
    };
    ...
    let final_data = if request.encrypt {
        if let Some(password) = request.password {
            encrypt_export_data(&data, &password)
                .map_err(|e| format!("エクスポートデータの暗号化に失敗しました: {}", e))?
        } else {
...
fn mask_sensitive_data(body: &str) -> String {
    ...
    if let Ok(mut json_value) = serde_json::from_str::<serde_json::Value>(body) {
        ...
        let sensitive_patterns = vec![
            ...
            (
                r#""access_token"\s*:\s*"[^"]*""#,
                r#""access_token": "***MASKED***""#,
            ),
            (
                r#""refresh_token"\s*:\s*"[^"]*""#,
                r#""refresh_token": "***MASKED***""#,
            ),
            (
                r#""bearer_token"\s*:\s*"[^"]*""#,
                r#""bearer_token": "***MASKED***""#,
            ),
            ...
            (
                r#""private_key"\s*:\s*"[^"]*""#,
                r#""private_key": "***MASKED***""#,
            ),
            // クレジットカード関連
            (
                r#""credit_card"\s*:\s*"[^"]*""#,
                r#""credit_card": "***MASKED***""#,
            ),
            (
                r#""card_number"\s*:\s*"[^"]*""#,
                r#""card_number": "***MASKED***""#,
            ),
...
pub async fn export_api_settings(request: ExportApiSettingsRequest) -> Result<String, String> {
    ...
    let apis = if let Some(api_ids) = request.api_ids {
        ...
        api_repo
            .find_all()
            .map_err(|e| format!("API一覧の取得に失敗しました: {e}"))?
    };

    // エクスポートデータに変換
    let settings_data: Vec<ApiSettingsData> = apis
        .into_iter()
        .map(|api| ApiSettingsData {
            id: api.id,
            name: api.name,
            model: api.model,
            port: api.port,
    ...
    // JSON形式でエクスポート
    serde_json::to_string_pretty(&export_data).map_err(|e| format!("JSON変換に失敗しました: {e}"))
}
...
#[tauri::command]
pub async fn import_api_settings(
    request: ImportApiSettingsRequest,
) -> Result<ImportApiSettingsResponse, String> {
    // JSONデータをパース
    let export_data: ApiSettingsExport = serde_json::from_str(&request.json_data)
                ...
                match request.conflict_resolution.as_str() {
                    ...
                    "overwrite" => {
                        ...
                        match api_repo.update(&crate::database::models::Api {
                            ...
                            timeout_secs: None,  // バックアップデータに含まれていない場合はNone
...
pub async fn delete_logs(request: DeleteLogsRequest) -> Result<DeleteLogsResponse, String> {
    ...
    // トランザクション内で削除を実行
    let deleted_count = log_repo
        .delete_by_date_range(request.api_id.as_deref(), request.before_date.as_deref())
        .map_err(|e| format!("ログの削除に失敗しました: {}", e))?;

    Ok(DeleteLogsResponse { deleted_count })
}
...
pub async fn get_model_catalog() -> Result<Vec<ModelCatalogInfo>, String> {
    ...
    let catalog_models = tokio::task::spawn_blocking(|| {
        ...
        catalog_repo
            .find_all()
            .map_err(|e| format!("モデルカタログの取得に失敗しました: {}", e))
    })
    .await
    .map_err(|e| format!("データベース操作エラー: {e}"))??;

    // ModelCatalogをModelCatalogInfoに変換
    let result: Vec<ModelCatalogInfo> = catalog_models
        .into_iter()
        .map(|model| ModelCatalogInfo {
            name: model.name,
...
#[tauri::command]
pub async fn get_huggingface_model_info(
    model_id: String,
) -> Result<crate::utils::huggingface::HuggingFaceModel, String> {
...
pub async fn get_security_settings(apiId: String) -> Result<Option<serde_json::Value>, String> {
    ...
    let settings = ApiSecuritySettingsRepository::find_by_api_id(&conn, &api_id)
        .map_err(|e| format!("セキュリティ設定の取得に失敗しました: {e}"))?;

    match settings {
        Some(settings) => {
            // IPホワイトリストをJSON配列に変換
            let ip_whitelist: Vec<String> = settings
                .ip_whitelist
                .as_ref()
...
pub async fn set_ip_whitelist(api_id: String, whitelist: Vec<String>) -> Result<(), String> {
    ...
    use crate::utils::input_validation;
    for ip in &whitelist {
        input_validation::validate_ip_address(ip)
            .map_err(|e| format!("IPアドレスの検証に失敗しました（{}）: {}", ip, e))?;
    }
    ...
    // 既存の設定を取得または新規作成
    let settings = match ApiSecuritySettingsRepository::find_by_api_id(&conn, &api_id)
        .map_err(|e| format!("セキュリティ設定の取得に失敗しました: {e}"))?
    {
        Some(mut s) => {
            // IPホワイトリストをJSON文字列に変換
            let ip_whitelist_json = serde_json::to_string(&whitelist)
                .map_err(|e| format!("IPホワイトリストの変換に失敗しました: {e}"))?;
    ...
    if settings.created_at == settings.updated_at && settings.created_at == Utc::now() {
        // 新規作成
        ApiSecuritySettingsRepository::create(&conn, &settings)
            .map_err(|e| format!("セキュリティ設定の保存に失敗しました: {e}"))?;
    } else {
        // 更新
        ApiSecuritySettingsRepository::update(&conn, &settings)
            .map_err(|e| format!("セキュリティ設定の更新に失敗しました: {e}"))?;
    }
...
pub async fn search_audit_logs(
...
) -> Result<Vec<crate::utils::audit_log::AuditLogEntry>, String> {
    use crate::utils::audit_log::{search_audit_logs, AuditAction};

    let action = request.action.as_deref().and_then(|a| match a {
    ...
    .await
    .map_err(|e| format!("監査ログの検索に失敗しました: {}", e))
}
...
pub async fn save_error_log(request: SaveErrorLogRequest) -> Result<(), String> {
    ...
    error_log_repo
        .create(&error_log)
        .map_err(|e| format!("エラーログの保存に失敗しました: {}", e))?;
...
pub async fn list_error_logs(request: ListErrorLogsRequest) -> Result<Vec<ErrorLogInfo>, String> {
    ...
    let error_log_infos: Vec<ErrorLogInfo> = error_logs
        .into_iter()
        .map(|log| ErrorLogInfo {
            id: log.id,
    ...
    let content = match format.as_str() {
        ...
        "json" => {
            ...
            let error_log_infos: Vec<ErrorLogInfo> = error_logs
                ...
                .collect();
            serde_json::to_string_pretty(&error_log_infos)
        ...
        _ => {
            return Err(
                "サポートされていない形式です。csv、json、txtのいずれかを指定してください。"
                    .to_string(),
            )
        }
    };
...
pub async fn export_audit_logs(request: ExportAuditLogsRequest) -> Result<String, String> {
    use crate::utils::audit_log::{export_audit_logs, AuditAction, ExportFormat};
    ...
    .await
    .map_err(|e| format!("監査ログのエクスポートに失敗しました: {}", e))
}
