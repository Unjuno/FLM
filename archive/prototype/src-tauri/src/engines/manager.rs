...
impl EngineManager {
    ...
    pub async fn start_engine(
        ...
        eprintln!(
            "[DEBUG] エンジン設定: エンジンタイプ={}, ベースURL={:?}, ポート={:?}, 自動検出={}, モデル名={:?}",
            config.engine_type, config.base_url, config.port, config.auto_detect, config.model_name
        );
        ...
        if let Some(port) = config.port {
            ...
            if !is_port_available(port) {
                eprintln!("[ERROR] ポート {} は既に使用されています", port);
                return Err(AppError::ApiError {
                    message: format!("ポート {} は既に使用されています。別のポートを指定するか、既存のプロセスを停止してください。", port),
                    code: "PORT_IN_USE".to_string(),
                    source_detail: Some(format!("エンジンタイプ: {}, ポート: {}", engine_type, port)),
                });
            }
            eprintln!("[DEBUG] ポート {} の使用可能性を確認しました（使用可能）", port);
        }
    ...
    pub async fn stop_engine(&self, engine_type: &str) -> Result<(), AppError> {
        eprintln!("[DEBUG] EngineManager::stop_engine 開始: engine_type={}", engine_type);
        let result = match engine_type {
            "ollama" => {
                let engine = OllamaEngine::new();
                engine.stop().await
        ...
        match &result {
            Ok(_) => {
                eprintln!("[DEBUG] EngineManager::stop_engine 成功: engine_type={}", engine_type);
            }
            Err(e) => {
                eprintln!("[ERROR] EngineManager::stop_engine 失敗: engine_type={}, error={}", engine_type, e);
            }
        }
        result
    }
