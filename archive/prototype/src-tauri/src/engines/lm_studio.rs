...
impl LLMEngine for LMStudioEngine {
    ...
    async fn detect(&self) -> Result<EngineDetectionResult, AppError> {
        ...
        let running = match self.is_running().await {
            Ok(r) => {
                eprintln!("[DEBUG] LM Studio検出: 起動状態確認成功 - running={}", r);
                r
            }
            Err(e) => {
                eprintln!("[WARN] LM Studio検出: 起動状態の確認に失敗しました: {}", e);
                false
            }
        };
    ...
    async fn stop(&self) -> Result<(), AppError> {
        eprintln!("[DEBUG] LM Studio停止処理を開始します");
        
        // まず、LM Studioが実行中かどうかを確認
        let was_running = match self.is_running().await {
            Ok(r) => r,
            Err(e) => {
                eprintln!("[WARN] LM Studioの実行状態確認に失敗しました: {}", e);
        ...
        if !was_running {
            eprintln!("[DEBUG] LM Studioは既に停止しています");
            return Ok(());
        }
        
        // LM StudioサーバーはHTTP APIで停止できないため、
        // ポートを使用しているプロセスを終了させる
        let port = self.default_port();
        eprintln!("[DEBUG] LM Studioサーバーを停止します: ポート={}", port);
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            // Windows: PowerShellを使用してポート1234を使用しているプロセスを停止
            let ps_cmd = format!(
                r#"
                $port = {}
                $stopped = $false
                    ...
                    foreach ($pid in $pids) {{
                        ...
                        if ($proc) {{
                            try {{
                                ...
                                if ($cmdLine -and ($cmdLine -like '*LM Studio*' -or $cmdLine -like '*lm-studio*' -or $procName -like '*LM Studio*' -or $procName -like '*lm-studio*')) {{
                                ...
                                $procName = $proc.ProcessName
                                if ($procName -like '*LM Studio*' -or $procName -like '*lm-studio*') {{
                                    Stop-Process -Id $pid -Force -ErrorAction SilentlyContinue
                ...
                if (-not $stopped) {{
                    Write-Output "No LM Studio process found on port $port (may already be stopped)"
                }}
                "#,
                port
            );
            ...
            match output {
                Ok(output) => {
                    ...
                    if output.status.success() || stdout.contains("Stopped") || stdout.contains("already be stopped") {
                        eprintln!("[DEBUG] LM Studio停止処理完了: {}", stdout.trim());
                        if stdout.contains("Stopped") {
                            process_found = true;
                        }
                    } else if !stderr.is_empty() {
                        eprintln!("[WARN] LM Studio停止コマンドの警告: {}", stderr.trim());
                    }
                    // プロセスが見つからない場合も既に停止している可能性があるため、続行
                }
                Err(e) => {
                    eprintln!("[ERROR] LM Studio停止コマンドの実行に失敗しました: {}", e);
                    // PowerShellコマンドの実行自体が失敗した場合のみエラーを返す
                    return Err(AppError::ProcessError {
                        message: format!("LM Studioプロセスの停止に失敗しました: {}", e),
            ...
            match output {
                Ok(output) if output.status.success() => {
                    ...
                    if let Ok(pid) = pid_str.parse::<u32>() {
                        eprintln!("[DEBUG] LM Studioプロセスを終了します: PID={}", pid);
                        ...
                        match kill_output {
                            Ok(kill_output) if kill_output.status.success() => {
                                eprintln!("[DEBUG] LM Studioプロセス (PID: {}) を停止しました", pid);
                                process_found = true;
                            }
                            Ok(_) => {
                                eprintln!("[WARN] LM Studioプロセス (PID: {}) の停止に失敗しました", pid);
                            }
                            Err(e) => {
                ...
                Ok(_) => {
                    ...
                    eprintln!("[DEBUG] LM Studioプロセスが見つかりませんでした（既に停止している可能性があります）");
                }
                Err(e) => {
                    eprintln!("[ERROR] lsofコマンドの実行に失敗しました: {}", e);
                    return Err(AppError::ProcessError {
                        message: format!("LM Studioプロセスの検索に失敗しました: {}", e),
        ...
        eprintln!("[DEBUG] LM Studio停止確認を開始します");
        for i in 0..6 {
            tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;
            match self.is_running().await {
                Ok(false) => {
                    eprintln!("[DEBUG] LM Studioの停止が確認できました");
                    return Ok(());
                }
                Ok(true) => {
                    eprintln!("[DEBUG] LM Studio停止確認試行 {}/6: まだ実行中", i + 1);
                }
                Err(e) => {
                    eprintln!("[WARN] LM Studio停止確認中にエラーが発生しました: {}", e);
                }
            }
        }
        ...
        eprintln!("[WARN] LM Studio停止コマンドは実行しましたが、3秒経過後もまだ実行中の可能性があります");
        Ok(())
    }
}
