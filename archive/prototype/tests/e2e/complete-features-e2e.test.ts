      ...
      try {
        ...
        if (models.length === 0) {
          if (
            ...
            console.warn(
              'ダウンロード可能なモデルがないため、ダウンロード進捗テストをスキップ'
            );
          }
        ...
        // 小さなモデルを選択（テスト用）
        const testModel =
          models.find(m => m.name && m.name.includes('small')) || models[0];
        ...
        try {
          ...
          const timeoutPromise = new Promise((_, reject) => {
            setTimeout(
              () => reject(new Error('ダウンロードタイムアウト（テスト用）')),
              10000
            );
          });
