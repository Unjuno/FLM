...
3. **APIテスト** (`tests/api/`)
   - APIエンドポイントのテスト
   - 例: 
     - `chat-api.test.ts` - チャットAPIのテスト
     - `audio-generation-api.test.ts` - 音声生成モデルのテスト
     - `image-generation-api.test.ts` - 画像生成モデルのテスト
     - `video-generation-api.test.ts` - 動画生成モデルのテスト
     - `code-generation-api.test.ts` - コード生成モデルのテスト
     - `embedding-api.test.ts` - 埋め込みモデルのテスト
     - `multi-category-models.test.ts` - 複数カテゴリモデルの統合テスト

4. **E2Eテスト** (`tests/e2e/`)
   - エンドツーエンドのフローテスト
   - 例: `api-creation-flow.test.ts`

5. **パフォーマンステスト** (`tests/performance/`)
   - アプリケーションのパフォーマンステスト
   - 例: `performance.test.ts`

6. **セキュリティテスト** (`tests/security/`)
   - セキュリティ関連のテスト
   - 例: `security.test.ts`
...
### 音声生成モデルのテストを実行
```bash
npm test -- tests/api/audio-generation-api.test.ts
```

### 画像生成モデルのテストを実行
```bash
npm test -- tests/api/image-generation-api.test.ts
```

### 動画生成モデルのテストを実行
```bash
npm test -- tests/api/video-generation-api.test.ts
```

### コード生成モデルのテストを実行
```bash
npm test -- tests/api/code-generation-api.test.ts
```

### 埋め込みモデルのテストを実行
```bash
npm test -- tests/api/embedding-api.test.ts
```

### 複数カテゴリモデルの統合テストを実行
```bash
npm test -- tests/api/multi-category-models.test.ts
```
