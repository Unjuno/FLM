...
pub async fn obtain_certificate(config: &LetsEncryptConfig) -> Result<CertificateInfo, AppError> {
    // 証明書保存ディレクトリを取得
    let cert_dir = get_certificate_directory(&config.api_id)?;
    fs::create_dir_all(&cert_dir)
        .map_err(|e| AppError::io_error(format!("証明書ディレクトリ作成エラー: {}", e)))?;
...
/// Let's Encrypt証明書を更新
pub async fn renew_certificate(config: &LetsEncryptConfig) -> Result<CertificateInfo, AppError> {
    // 証明書の有効期限を確認
...
/// 証明書情報を取得
pub async fn get_certificate_info(api_id: &str, domain: &str) -> Result<CertificateInfo, AppError> {
...
fn get_certificate_directory(api_id: &str) -> Result<PathBuf, AppError> {
    ...
    let app_data_dir = get_app_data_dir().map_err(|e| {
        AppError::database_error(format!("アプリデータディレクトリ取得エラー: {}", e))
    })?;
...
/// 証明書の自動更新をスケジュール
pub async fn schedule_certificate_renewal(config: LetsEncryptConfig) -> Result<(), AppError> {
    // スケジューラーに証明書更新タスクを追加
...
/// 証明書の有効期限をチェック
pub async fn check_certificate_expiry(api_id: &str, domain: &str) -> Result<bool, AppError> {
