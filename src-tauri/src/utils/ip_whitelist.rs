...
pub fn is_ip_allowed(ip_address: &str, whitelist: &[String]) -> Result<bool, AppError> {
    if whitelist.is_empty() {
        // ホワイトリストが空の場合は、すべてのIPを許可
        return Ok(true);
    }

    let client_ip: IpAddr = ip_address.parse().map_err(|e| AppError::ApiError {
    ...
    for entry in whitelist {
        if entry.contains('/') {
            ...
            // 単一IPアドレス
            let whitelist_ip: IpAddr = entry.parse().map_err(|e| AppError::ApiError {
...
fn match_cidr(ip: &IpAddr, cidr: &str) -> Result<bool, AppError> {
    ...
    let network_ip: IpAddr = parts[0].parse().map_err(|e| AppError::ApiError {
        message: format!("CIDRネットワークIP解析エラー: {}", e),
        code: "INVALID_CIDR_IP".to_string(),
        source_detail: None,
    })?;

    let _prefix_len: u8 = parts[1].parse().map_err(|e| AppError::ApiError {
