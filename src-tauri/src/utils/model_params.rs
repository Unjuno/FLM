    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ValidationError::TemperatureOutOfRange(val) => {
                write!(
                    f,
                    "温度(Temperature)の値{}は範囲外です。{}-{}の範囲で指定してください。",
                    val,
                    constants::model_parameters::TEMPERATURE_MIN,
                    constants::model_parameters::TEMPERATURE_MAX
                )
            }
            ValidationError::TopPOutOfRange(val) => {
                write!(
                    f,
                    "Top-pの値{}は範囲外です。{}-{}の範囲で指定してください。",
                    val,
                    constants::model_parameters::TOP_P_MIN,
                    constants::model_parameters::TOP_P_MAX
                )
            }
            ValidationError::TopKOutOfRange(val) => {
                write!(
                    f,
                    "Top-kの値{}は範囲外です。{}-{}の範囲で指定してください。",
                    val,
                    constants::model_parameters::TOP_K_MIN,
                    constants::model_parameters::TOP_K_MAX
                )
            }
            ValidationError::MaxTokensInvalid(val) => {
                write!(
                    f,
                    "最大トークン数(Max Tokens)の値{}は無効です。{}以上の値を指定してください。",
                    val,
                    constants::model_parameters::MAX_TOKENS_MIN
                )
            }
            ...
            ValidationError::NumGpuLayersOutOfRange(val) => {
                write!(
                    f,
                    "GPUレイヤー数の値{}は範囲外です。{}-{}の範囲で指定してください。",
                    val,
                    constants::memory_settings::NUM_GPU_LAYERS_MIN,
                    constants::memory_settings::NUM_GPU_LAYERS_MAX
                )
            }
            ValidationError::NumThreadsOutOfRange(val) => {
                write!(
                    f,
                    "CPUスレッド数の値{}は範囲外です。{}-{}の範囲で指定してください。",
                    val,
                    constants::memory_settings::NUM_THREADS_MIN,
                    constants::memory_settings::NUM_THREADS_MAX
                )
            }
            ValidationError::BatchSizeOutOfRange(val) => {
                write!(
                    f,
                    "バッチサイズの値{}は範囲外です。{}-{}の範囲で指定してください。",
                    val,
                    constants::memory_settings::BATCH_SIZE_MIN,
                    constants::memory_settings::BATCH_SIZE_MAX
                )
            }
            ValidationError::MaxImageSizeOutOfRange(val) => {
                write!(
                    f,
                    "最大画像サイズの値{}は範囲外です。{}-{}の範囲で指定してください。",
                    val,
                    constants::multimodal_settings::MAX_IMAGE_SIZE_MIN,
                    constants::multimodal_settings::MAX_IMAGE_SIZE_MAX
                )
            }
            ValidationError::MaxAudioSizeOutOfRange(val) => {
                write!(
                    f,
                    "最大音声サイズの値{}は範囲外です。{}-{}の範囲で指定してください。",
                    val,
                    constants::multimodal_settings::MAX_AUDIO_SIZE_MIN,
                    constants::multimodal_settings::MAX_AUDIO_SIZE_MAX
                )
            }
            ValidationError::MaxVideoSizeOutOfRange(val) => {
                write!(
                    f,
                    "最大動画サイズの値{}は範囲外です。{}-{}の範囲で指定してください。",
                    val,
                    constants::multimodal_settings::MAX_VIDEO_SIZE_MIN,
                    constants::multimodal_settings::MAX_VIDEO_SIZE_MAX
                )
            }
