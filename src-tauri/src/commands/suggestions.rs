...
pub async fn suggest_error_fix(error_message: String) -> Result<ErrorFixSuggestion, String> {
    ...
    if error_lower.contains("port") || error_lower.contains("ポート") {
        return Ok(ErrorFixSuggestion {
            ...
            fix_description: "ポート番号が既に使用されています。別のポート番号を自動検出して提案します。".to_string(),
            can_auto_fix: true,
    ...
    // モデルが見つからないエラー
    if error_lower.contains("model") && (error_lower.contains("not found") || error_lower.contains("見つかりません")) {
        return Ok(ErrorFixSuggestion {
            error_type: "model_not_found".to_string(),
            error_message: error_message.clone(),
            fix_description: "指定されたモデルが見つかりません。モデル管理画面からダウンロードしてください。".to_string(),
            can_auto_fix: false,
            fix_action: None,
        });
    }
    ...
    if error_lower.contains("engine") || error_lower.contains("エンジン") {
        return Ok(ErrorFixSuggestion {
            ...
            error_message: error_message.clone(),
            fix_description: "LLMエンジンの起動に失敗しました。エンジンの状態を確認してください。".to_string(),
            can_auto_fix: false,
            fix_action: None,
        });
    }
    ...
    if error_lower.contains("auth") || error_lower.contains("認証") {
        return Ok(ErrorFixSuggestion {
            ...
            error_message: error_message.clone(),
            fix_description: "認証設定に問題があります。APIキーを再生成してみてください。".to_string(),
            can_auto_fix: false,
            fix_action: None,
        });
    }
    ...
    Ok(ErrorFixSuggestion {
        ...
        error_message: error_message.clone(),
        fix_description: "エラーの詳細を確認してください。アプリケーションを再起動してみることをお勧めします。".to_string(),
        can_auto_fix: false,
        fix_action: None,
    })
}
...
pub async fn suggest_model_parameters(
    ...
    // システムリソースを取得
    let resources = get_system_resources().await.map_err(|e| {
        format!("システムリソースの取得に失敗しました: {}", e)
    })?;
    ...
    let use_case_type = match use_case.as_deref() {
        ...
        Some("conversational") | Some("対話型") | Some("チャット") => UseCaseType::Conversational,
...
/// システムリソースに基づくメモリ設定の推奨
fn suggest_memory_settings(resources: &SystemResources, model_name: Option<&str>) -> MemoryRecommendation {
