...
impl<'a> ApiRepository<'a> {
    ...
    pub fn update(&self, api: &Api) -> Result<(), DatabaseError> {
        ...
        if rows_affected == 0 {
            return Err(DatabaseError::NotFound(format!("API ID '{}' が見つかりません", api.id)));
        }
    ...
    pub fn delete(&self, id: &str) -> Result<(), DatabaseError> {
        ...
        if rows_affected == 0 {
            return Err(DatabaseError::NotFound(format!("API ID '{}' が見つかりません", id)));
        }
...
impl<'a> ApiKeyRepository<'a> {
    ...
    pub fn update(&self, api_key: &ApiKey) -> Result<(), DatabaseError> {
        ...
        if rows_affected == 0 {
            return Err(DatabaseError::NotFound(format!("API Key ID '{}' が見つかりません", api_key.id)));
        }
    ...
    pub fn delete_by_api_id(&self, api_id: &str) -> Result<(), DatabaseError> {
        self.conn.execute(
            "DELETE FROM api_keys WHERE api_id = ?1",
            params![api_id],
        )?;
        Ok(())
    }
        ...
        let models = stmt.query_map([], |row| {
            Ok(InstalledModel {
                ...
                installed_at: chrono::DateTime::parse_from_rfc3339(&row.get::<_, String>(3)?)
                    .map_err(|_| rusqlite::Error::InvalidColumnType(3, "installed_at".to_string(), rusqlite::types::Type::Text))?
                    .with_timezone(&Utc),
                last_used_at: row.get::<_, Option<String>>(4)?
                    .map(|s| chrono::DateTime::parse_from_rfc3339(&s)
                        .map_err(|_| rusqlite::Error::InvalidColumnType(4, "last_used_at".to_string(), rusqlite::types::Type::Text))
...
impl<'a> RequestLogRepository<'a> {
    ...
    pub fn find_with_filters(
        ...
        query.push_str(&format!(" LIMIT ?{} OFFSET ?{}", limit_param_idx, offset_param_idx));
    ...
    pub fn get_statistics(
        ...
        // 総リクエスト数
        let total_requests_query = format!(
        ...
        let total_requests: i64 = if param_refs.is_empty() {
            self.conn.query_row(&total_requests_query, [], |row| row.get(0))?
        } else {
            self.conn.query_row(&total_requests_query, params_from_iter(param_refs.iter()), |row| row.get(0))?
        };
        
        // 平均レスポンス時間（response_time_msがNULLでないもののみ）
        let avg_response_time_query = if conditions.is_empty() {
            "SELECT AVG(response_time_ms) FROM request_logs WHERE response_time_ms IS NOT NULL".to_string()
        } else {
            format!(
        ...
        let avg_response_time_ms: f64 = if param_refs.is_empty() {
            self.conn.query_row(&avg_response_time_query, [], |row| {
                Ok(row.get::<_, Option<f64>>(0)?.unwrap_or(0.0))
            }).unwrap_or(0.0)
        } else {
            self.conn.query_row(&avg_response_time_query, params_from_iter(param_refs.iter()), |row| {
                Ok(row.get::<_, Option<f64>>(0)?.unwrap_or(0.0))
            }).unwrap_or(0.0)
        };
        ...
        let error_count: i64 = if param_refs.is_empty() {
            self.conn.query_row(&error_count_query, [], |row| row.get(0))?
        } else {
            self.conn.query_row(&error_count_query, params_from_iter(param_refs.iter()), |row| row.get(0))?
        };
        ...
        let settings = stmt.query_map([], |row| {
            Ok(UserSetting {
                ...
                updated_at: chrono::DateTime::parse_from_rfc3339(&row.get::<_, String>(2)?)
                    .map_err(|_| rusqlite::Error::InvalidColumnType(2, "updated_at".to_string(), rusqlite::types::Type::Text))?
                    .with_timezone(&Utc),
            })
        })?;
...
impl<'a> PerformanceMetricRepository<'a> {
    ...
    pub fn find_by_api_id(&self, api_id: &str, limit: Option<i32>) -> Result<Vec<PerformanceMetric>, DatabaseError> {
    ...
    pub fn find_by_api_id_and_type(&self, api_id: &str, metric_type: &str, limit: Option<i32>) -> Result<Vec<PerformanceMetric>, DatabaseError> {
        let limit_value = limit.unwrap_or(1000);
        let mut stmt = self.conn.prepare(
    ...
    pub fn find_by_api_id_and_range(&self, api_id: &str, start_date: Option<&str>, end_date: Option<&str>, metric_type: Option<&str>) -> Result<Vec<PerformanceMetric>, DatabaseError> {
...
mod tests {
    ...
    fn test_api_key_repository_create_and_find() {
        ...
        let found = key_repo.find_by_api_id("test-api-1")
            .expect("APIキー取得に失敗")
    ...
    fn test_installed_model_repository() {
        ...
        repo.increment_usage("llama3:8b").expect("使用回数更新に失敗");
    ...
    fn test_user_setting_repository() {
        ...
        repo.set("test_key", "updated_value").expect("設定更新に失敗");
