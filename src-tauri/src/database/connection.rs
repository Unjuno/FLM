...
// マクロをインポート
use crate::{debug_log, warn_log, error_log};
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let app_data_dir = match env::var("LOCALAPPDATA") {
            ...
            Err(_) => {
                ...
                warn_log!("警告: LOCALAPPDATA環境変数が取得できませんでした。代替パスを使用します。");
                match env::var("USERPROFILE") {
                    Ok(user_profile) => {
                        debug_log!("USERPROFILE環境変数を取得: {}", mask_env_var_value(&user_profile));
                        PathBuf::from(user_profile).join("AppData").join("Local").join("FLM")
                    },
                    Err(_) => {
                        // 最後の手段: カレントディレクトリ
...
pub fn get_connection() -> Result<DatabaseConnection, crate::database::DatabaseError> {
    ...
    let db_path = match get_database_path() {
        Ok(path) => {
            debug_log!("データベースパス: {:?}", mask_file_path(&path.to_string_lossy()));
            path
        },
        Err(e) => {
            error_log!("データベースパス取得エラー: {:?}", e);
            return Err(crate::database::DatabaseError::ConnectionFailed(
                format!("データベースの保存先を取得できませんでした: {}", e)
            ));
        }
    };
    ...
    if let Some(parent) = db_path.parent() {
        debug_log!("データベースディレクトリ: {:?}", mask_file_path(&parent.to_string_lossy()));
        match std::fs::create_dir_all(parent) {
            Ok(_) => {
                debug_log!("データベースディレクトリを作成しました: {:?}", mask_file_path(&parent.to_string_lossy()));
            },
            Err(e) => {
                error_log!("ディレクトリ作成エラー: {:?} (パス: {:?})", e, mask_file_path(&parent.to_string_lossy()));
                return Err(crate::database::DatabaseError::ConnectionFailed(
        ...
        warn_log!("警告: データベースパスに親ディレクトリがありません: {:?}", mask_file_path(&db_path.to_string_lossy()));
    }
    
    // データベース接続を開く
    debug_log!("データベース接続を開いています: {:?}", mask_file_path(&db_path.to_string_lossy()));
    let conn = match Connection::open(&db_path) {
        ...
        Err(e) => {
            error_log!("データベース接続エラー: {:?} (パス: {:?})", e, mask_file_path(&db_path.to_string_lossy()));
            return Err(crate::database::DatabaseError::ConnectionFailed(
    ...
    if let Err(e) = conn.execute("PRAGMA page_size = 8192", []) {
        debug_log!("ページサイズの設定: 既存データベースのため変更されません（エラーは無視）: {:?}", e);
    }
