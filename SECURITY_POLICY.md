...
FLMアプリケーションは、以下の外部APIサービスに接続する可能性があります：

**重要**: すべての外部API接続は、ユーザーの明示的な同意または操作に基づいて行われます。自動的なデータ送信は行われません。

#### 1. GitHub API
- **用途**: 
  - Ollamaの自動ダウンロード（初回起動時、未インストール時のみ）
  - 設定のクラウド同期機能（オプション、将来実装予定）
- **接続先**: `https://api.github.com`
- **送信データ**: 
  - Ollamaダウンロード時: リリース情報の取得のみ（GETリクエスト）
  - クラウド同期時: 設定ファイル（暗号化済み）
- **認証**: 不要（公開API、Ollamaダウンロード時）、またはユーザーが提供するGitHubアクセストークン（クラウド同期時）
- **プライバシー**: 
  - Ollamaダウンロード: 初回起動時のみ自動接続（ユーザー操作なし）
  - クラウド同期: ユーザーが明示的に有効化した場合のみ接続
- **通知**: Ollamaダウンロード時は、ダウンロード進捗がUIに表示されます

#### 2. Hugging Face Hub API
- **用途**: モデル情報の取得、モデルダウンロード機能（オプション）
- **接続先**: `https://huggingface.co`、`https://*.huggingface.co`
- **送信データ**: 
  - モデル検索時: 検索クエリ（モデル名、タグなど）
  - モデルダウンロード時: モデルメタデータ、モデルファイル
- **認証**: 不要（公開API）
- **プライバシー**: ユーザーがモデル検索またはダウンロードを実行した場合のみ接続
- **通知**: モデル検索時は、検索結果がUIに表示されます

#### 3. Google Drive API（将来の機能）
- **用途**: 設定のクラウド同期機能（オプション、将来実装予定）
- **接続先**: `https://www.googleapis.com`
- **送信データ**: 設定ファイル（暗号化済み）
- **認証**: ユーザーが提供するGoogleアクセストークン
- **プライバシー**: ユーザーが明示的に有効化した場合のみ接続

#### 4. Dropbox API（将来の機能）
- **用途**: 設定のクラウド同期機能（オプション、将来実装予定）
- **接続先**: `https://api.dropbox.com`
- **データ**: 設定ファイル（暗号化済み）
- **認証**: ユーザーが提供するDropboxアクセストークン
- **プライバシー**: ユーザーが明示的に有効化した場合のみ接続

### データの取り扱い

- **ローカル保存**: すべての設定データはローカルに保存されます
- **暗号化**: APIキーや機密情報はAES-256-GCMで暗号化して保存されます
- **オプトイン**: クラウド同期機能はユーザーが明示的に有効化した場合のみ動作します
- **データ送信**: クラウド同期を使用する場合、設定ファイルのみが送信されます（APIキーは含まれません）

### プライバシー保護

- **デバイスIDのハッシュ化**: クラウド同期時にデバイスIDはSHA256でハッシュ化されます
- **暗号化通信**: すべての外部API通信はHTTPSで暗号化されます
- **最小限のデータ送信**: 必要最小限のデータのみが送信されます
