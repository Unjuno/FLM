...
describe('F006: ログ表示機能 E2Eテスト', () => {
    ...
    try {
      ...
      const logsToCreate = [
        ...
        },
        {
          method: 'POST',
          path: '/v1/chat/completions',
          request_body: JSON.stringify({ messages: [{ role: 'user', content: 'Error test' }] }),
          response_status: 500,
        ...
        },
        {
          method: 'POST',
          path: '/v1/chat/completions',
          request_body: JSON.stringify({ messages: [{ role: 'user', content: 'Another test' }] }),
          response_status: 200,
  ...
  describe('統計情報表示フロー', () => {
    ...
    it('should update statistics when filters are applied', async () => {
      ...
      expect(filteredStats.total_requests).toBeLessThanOrEqual(allStats.total_requests);
  ...
  describe('完全なフロー: ログ一覧 → フィルタ → 詳細表示 → 統計表示', () => {
    it('should complete full workflow from list to statistics', async () => {
      ...
      // ステップ5: ステータスコード分布を確認
      expect(Array.isArray(stats.status_code_distribution)).toBe(true);
      const status200Count = stats.status_code_distribution.find(([status]) => status === 200)?.[1] || 0;
