...
describe('ApiCreationProgress.tsx', () => {
  ...
  describe('基本的なレンダリング', () => {
    ...
    it('進捗パーセンテージを表示する', () => {
      ...
      expect(screen.getByText(/75|%|パーセント/i)).toBeInTheDocument();
    });
  });
  ...
  describe('進捗状態の表示', () => {
    it('進捗0%の場合、初期状態を表示する', () => {
      ...
      expect(screen.getByText(/0|%|開始/i)).toBeInTheDocument();
    });
    ...
    it('進捗50%の場合、中間状態を表示する', () => {
      ...
      expect(screen.getByText(/50|%|処理中/i)).toBeInTheDocument();
    });
    ...
    it('進捗100%の場合、完了状態を表示する', () => {
      ...
      expect(screen.getByText(/100|%|完了/i)).toBeInTheDocument();
    });
  });
  ...
  describe('ステップ表示', () => {
    ...
    it('進捗ステップを表示する', () => {
      ...
      expect(
        screen.getByText(/エンジン確認|設定保存|認証プロキシ起動|完了/i)
      ).toBeInTheDocument();
    });
  });
});
