...
// webModelConfig.tsをモックしてimport.meta.envを回避
jest.mock('../../src/utils/webModelConfig', () => {
  const actual = jest.requireActual('../../src/utils/webModelConfig');

  // import.meta.env.DEVを使っている部分をモック
  return {
    ...actual,
    loadWebModelConfig: jest.fn(async (customPath?: string) => {
      // 実際の実装をシミュレート（import.meta.env.DEVの部分をスキップ）
      try {
        const configPath = customPath || '/assets/web-models-config.json';
        const response = await fetch(configPath);
        if (!response.ok) {
          throw new Error(
            `設定ファイルの読み込みに失敗: ${response.statusText}`
          );
        }
        const config = await response.json();
        actual.validateConfig(config);
        return config;
      } catch (error) {
        // DEVモードのログはスキップ
        throw new Error(
...
describe('webModelConfig.ts', () => {
  ...
  describe('validateConfig関数', () => {
    it('有効な設定を検証する', () => {
      ...
      const { validateConfig } = require('../../src/utils/webModelConfig');
  ...
  describe('filterByCategory関数', () => {
    it('カテゴリでフィルタリングする', () => {
      ...
      const { filterByCategory } = require('../../src/utils/webModelConfig');
  ...
  describe('getRecommendedModels関数', () => {
    it('推奨モデルのみを取得する', () => {
      ...
      } = require('../../src/utils/webModelConfig');
  ...
  describe('findModelById関数', () => {
    it('IDでモデルを検索する', () => {
      ...
      const { findModelById } = require('../../src/utils/webModelConfig');
  ...
  describe('findModelByName関数', () => {
    it('モデル名とエンジンでモデルを検索する', () => {
      ...
      const { findModelByName } = require('../../src/utils/webModelConfig');
