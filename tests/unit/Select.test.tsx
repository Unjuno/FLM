...
describe('Select.tsx', () => {
  ...
  describe('エラーハンドリング', () => {
    it('エラーメッセージを表示する', async () => {
      ...
      const errorMessage = container.querySelector('.form-select-error-message');
    ...
    it('エラー時はヘルプテキストを表示しない', async () => {
      const options: SelectOption[] = [{ value: '1', label: 'オプション1' }];
      root.render(<Select options={options} error="エラー" helpText="ヘルプ" />);
      await waitForRender();

      expect(container.querySelector('.form-select-error-message')).toBeTruthy();
  ...
  describe('無効化状態', () => {
    ...
    it('readOnly時はonChangeが無効化される', async () => {
      ...
      root.render(<Select options={options} readOnly value="1" onChange={handleChange} />);
      await waitForRender();
    ...
    it('readOnlyがfalseでpropsOnChangeが存在する場合、onChangeが正常に呼ばれる', async () => {
      ...
      root.render(<Select options={options} readOnly={false} onChange={handleChange} />);
      await waitForRender();
      ...
      const callArgs = handleChange.mock.calls[0][0] as React.ChangeEvent<HTMLSelectElement>;
  ...
  describe('フル幅', () => {
    it('fullWidthでレンダリングできる', async () => {
      ...
      const wrapper = container.querySelector('.form-select-wrapper-full-width');
  ...
  describe('アクセシビリティ', () => {
    ...
    it('aria-labelledby属性が設定される（ラベルがある場合）', async () => {
      const options: SelectOption[] = [{ value: '1', label: 'オプション1' }];
      root.render(<Select label="テストラベル" options={options} id="test-select" />);
      await waitForRender();
    ...
    it('aria-describedby属性が設定される（ヘルプテキストがある場合）', async () => {
      const options: SelectOption[] = [{ value: '1', label: 'オプション1' }];
      root.render(<Select options={options} helpText="ヘルプ" id="test-select" />);
      await waitForRender();
