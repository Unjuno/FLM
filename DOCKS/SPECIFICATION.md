...
1. **モデル選択**
   - [x] 利用可能なLLMモデルリストを表示
   - [x] モデル名、サイズ、説明を表示
   - [x] モデル未インストール時はダウンロードオプション表示
   - [x] 推奨モデルのハイライト表示

2. **設定**
   - [x] API名の入力（オプション、デフォルト: "LocalAI API"）
   - [x] ポート番号の指定（デフォルト: 8080）
   - [x] 認証設定（APIキー有効化オプション）

3. **エンジン確認・インストール**
   - [x] API作成前に選択されたエンジンのインストール状態を自動確認
   - [x] **Ollamaの場合**: 未インストール時は自動インストールを実行（F009参照）
   - [x] **その他エンジンの場合**: 未インストール時は手動インストールを促すメッセージ表示
   - [x] インストール中の進捗表示（Ollamaのみ）

4. **作成・デプロイ**
   - [x] 「APIを作成」ボタンのクリック
   - [x] プログレスバーで進行状況を表示
   - [x] 成功時にAPIエンドポイント情報を表示
   - [x] エラー時はわかりやすいエラーメッセージ表示
...
エンジン検出（未インストール時の処理）
  ├─ Ollama: 自動インストール
  └─ その他: 手動インストールを促すメッセージ表示
  ↓
モデル選択画面（選択したエンジンのモデル一覧）
  ↓
設定画面（オプション）
  ↓
「作成」ボタン
...
1. **API一覧表示**
   - [x] 作成済みAPIのリスト表示
   - [x] API名、ステータス（実行中/停止）、エンドポイントURL表示
   - [x] 起動/停止ボタン

2. **APIテスト**
   - [x] テスト用のシンプルなチャットインターフェース
   - [x] プロンプト入力欄
   - [x] 送信ボタン
   - [x] レスポンス表示エリア
   - [x] タイムスタンプ、トークン数表示（オプション）

3. **API情報表示**
   - [x] エンドポイントURL
   - [x] APIキー（存在する場合、表示/非表示切り替え）
   - [x] サンプルコード（curl、Python、JavaScript）
   - [x] 「コピー」ボタン
...
2. **設定変更**
   - [x] ポート番号の変更
   - [x] 認証設定の変更（APIキーの再生成等）

3. **削除**
   - [x] 削除ボタン
   - [x] 確認ダイアログ
   - [x] 関連リソース（モデル等）の削除オプション

4. **アプリケーション終了時のクリーンアップ**
   - [x] アプリケーション終了時に、実行中のすべてのAPIを自動的に停止する（デフォルト動作）
   - [x] 認証プロキシプロセス（Node.jsプロセス）を確実に終了させる
   - [x] データベースのステータスを「stopped」に更新する
   - [x] クリーンアップ処理は非同期で実行し、最大10秒のタイムアウトを設定する
   - [x] クリーンアップ処理中のエラーはログに記録するが、アプリケーション終了をブロックしない
   - [x] **オプション機能**: アプリ終了時にAPIを停止するかどうかを設定で制御可能
     - 設定項目: 「アプリ終了時にAPIを停止する」（デフォルト: 有効）
     - 設定場所: 設定ページ > 「アプリ終了時の動作」セクション
     - 有効時: アプリ終了時にすべてのAPIを自動停止（従来の動作）
     - 無効時: アプリ終了時もAPIを実行し続ける（アプリを閉じてもAPIは継続実行）
     - 実装: 設定値は`user_settings`テーブルの`stop_apis_on_exit`キーで管理
...
1. **モデル検索機能（強化）**
   - [x] **リアルタイム検索バー**: モデル名、説明文からキーワード検索
   - [x] **カテゴリフィルタ**: 
     - チャット/会話
     - コード生成
     - 翻訳
...
6. **Hugging Face統合機能（v2.0実装済み）**
   - [x] **Hugging Faceモデル検索**:
     - Hugging Face APIとの統合
     - モデル名、説明、サイズでの検索
     - モデル詳細情報の取得
   - [x] **Hugging Faceからのモデルダウンロード**:
     - Hugging Face Hubからのモデルダウンロード
     - ダウンロード進捗の表示
     - エラーハンドリング

7. **Modelfile作成支援機能（v2.0実装済み）**
   - [x] **GUIでのModelfile作成・編集**:
     - モデルパラメータの設定
     - システムプロンプトの設定
     - テンプレートの選択
   - [x] **Modelfileの保存・読み込み**:
...
9. **モデル共有機能（v2.0部分実装）**
   - [x] **モデル共有UI**:
     ...
     - 共有モデル情報の表示
   - [ ] **実際のプラットフォーム連携**:
     - Hugging Face Hubへのアップロード（将来実装予定）
     - Ollama Hubへのアップロード（将来実装予定）
...
1. **Ollama検出**
   - [x] アプリ起動時にOllamaのインストール状態を自動検出
   - [x] `ollama --version` コマンドで確認（システムパス上のOllama）
   - [x] `http://localhost:11434/api/version` で確認（実行中のOllama）
   - [x] FLMアプリディレクトリ内のポータブル版Ollamaを確認（優先）
   - [x] 未検出の場合は自動セットアップを開始（確認ダイアログなし、自動実行）

2. **自動セットアップ（権限不要方式）**
   
   **優先方式1: ポータブル版Ollamaの使用（推奨・デフォルト）**
   - [x] FLMアプリのインストールディレクトリ内にOllamaを配置
   ...
   - [x] ダウンロードURL: Ollama公式リリースページから最新版を取得
     - GitHub Releases APIを使用: `https://api.github.com/repos/ollama/ollama/releases/latest`
   - [x] ダウンロード進捗表示（プログレスバー、パーセンテージ、速度）
   - [x] ダウンロード完了後、自動的に実行権限を設定（Unix系OS）
   - [x] 初回起動の自動実行（バックグラウンドプロセスとして起動）
   ...
   **優先方式3: ポータブル版自動ダウンロード（フォールバック）**
   - [x] バンドル版が利用できない場合のみ使用
   - [x] 初回起動時にインターネット経由でダウンロード
   - [x] 2回目以降はダウンロード不要
...
3. **セットアップ後の自動起動**
   - [x] FLMアプリ起動時に、ポータブル版Ollamaを自動起動
   - [x] バックグラウンドプロセスとして管理（Tauriバックエンド）
   - [x] Ollamaプロセスの状態監視（起動失敗時の自動リトライ）
   - [x] FLMアプリ終了時にOllamaプロセスも終了（オプション、設定で変更可能）
...
4. **エラーハンドリング（非開発者向け）**
   ...
   - [x] エラーメッセージは専門用語を使わず、具体的なアクションを提示
   - [x] **エンジン自動起動機能（強化）**:
     - エンジン選択時に自動的にエンジンを検出・起動
     - Ollama、LM Studio、vLLM、llama.cppすべてに対応
     - 起動中はUIを無効化して連打を防止
     - 起動中のローディング表示とメッセージ表示
...
#### 2.1.5.1 Ollama自動更新機能

**機能ID**: F009-EXT  
**優先度**: 中（実装済み）  
**概要**: Ollamaの最新版チェックと自動更新

**詳細仕様**:

1. **更新チェック**
   - [x] 現在インストールされているOllamaのバージョン確認
   - [x] GitHub Releases APIから最新版を取得
   - [x] 更新の必要性を判定
   - [x] 更新情報の表示（現在のバージョン、最新バージョン）

2. **自動更新**
   - [x] 既存のOllamaプロセスの停止
   - [x] 最新版のダウンロード
   - [x] 更新進捗の表示
   - [x] 更新後のOllamaの自動起動（元々実行中だった場合）
   - [x] 更新完了の通知
...
**機能ID**: F018  
**優先度**: 中（実装済み）  
**概要**: サードパーティプラグインの登録・管理・実行

**詳細仕様**:

1. **プラグイン登録**
   - [x] プラグイン情報の登録（名前、バージョン、作者、説明）
   - [x] プラグインタイプの設定
   - [x] プラグインの有効化/無効化
   - [x] プラグインの削除
...
1. **スケジュールタスクの設定**
   - [x] タスクタイプの選択（モデルカタログ更新、自動バックアップ、証明書更新等）
   - [x] 実行間隔の設定（秒単位）
   - [x] タスクの有効化/無効化
   - [x] タスクの追加・更新・削除
...
#### 2.1.8 WebServiceSetup機能

**機能ID**: F020  
**優先度**: 低（実装済み）  
**概要**: Webサービスの要件に基づく自動モデル選択とAPI作成

**詳細仕様**:

1. **要件入力**
   - [x] Webサービスの要件入力（用途、応答時間、精度等）
   - [x] システムリソースの自動検出
   - [x] 要件に基づく推奨設定の表示

2. **モデル自動選択**
   - [x] システムリソースに基づくモデル自動選択
   - [x] 要件に適合するモデルの提案
   - [x] モデル選択理由の表示
...
**注意**: 本機能は**シングルユーザー環境専用**です。マルチユーザー対応やロールベースアクセス制御（RBAC）機能は提供されていません。
...
**機能ID**: F016  
**優先度**: 中（実装済み）  
**概要**: OAuth 2.0認証の設定と管理

**詳細仕様**:

1. **OAuth設定**
   - [x] OAuth 2.0プロバイダーの設定（Client ID、Client Secret）
   - [x] 認証エンドポイントの設定
   - [x] トークンエンドポイントの設定
   - [x] リダイレクトURIの設定
   - [x] スコープの設定
...
**機能ID**: F013  
**優先度**: 高（v1.1実装済み）  
**概要**: パフォーマンス低下やエラー発生を監視し、アラートを通知
...
3. **ログ統計情報**
   - [x] ログ統計情報の表示
   - [x] リクエスト数、エラー率、平均レスポンス時間の集計

#### 2.3.5 監査ログ機能

**機能ID**: F014  
**優先度**: 高（実装済み）  
**概要**: すべてのAPI操作の記録を表示・管理
...
1. **バックアップ作成**
   - [x] API設定のバックアップ（JSON形式）
   - [x] モデル情報のバックアップ
   - [x] ログデータのバックアップ（オプション）
   - [x] バックアップファイルの保存先選択
   - [x] バックアップ情報の表示（API数、モデル数、ログ数など）
...
3. **設定のエクスポート/インポート**
   - [x] API設定のJSON形式でのエクスポート
   - [x] エクスポートした設定のインポート
   - [x] 設定の比較機能

#### 2.3.7 アプリケーション自動アップデート機能

**機能ID**: F020  
**優先度**: 最高（セキュリティ要件）  
**概要**: アプリケーション本体の自動アップデート機能（セキュリティパッチの迅速な適用）

**詳細仕様**:

1. **アップデートチェック**
   - [ ] アプリケーション起動時の自動チェック（オプション）
   - [ ] 手動チェック機能（設定画面から実行可能）
   - [ ] 定期的な自動チェック（設定可能、デフォルト: 24時間ごと）
   - [ ] 現在のバージョンと最新バージョンの比較
   - [ ] アップデート情報の表示（リリースノート、変更内容）

2. **アップデート通知**
   - [ ] アップデート利用可能時の通知表示
   - [ ] セキュリティアップデートの優先表示
   - [ ] アップデートの重要度表示（必須/推奨/オプション）
   - [ ] 通知の非表示設定（ユーザーが後で確認できるように）

3. **アップデート実行**
   - [ ] アップデートのダウンロード（バックグラウンド）
   - [ ] ダウンロード進捗の表示
   - [ ] アップデートファイルの整合性検証（署名検証、チェックサム検証）
   - [ ] アップデートインストール前の確認
   - [ ] アプリケーション再起動による自動インストール
   - [ ] アップデート失敗時のロールバック機能

4. **セキュリティ**
   - [ ] アップデートサーバーからの署名検証（必須）
   - [ ] ダウンロードファイルのチェックサム検証
   - [ ] HTTPS通信の強制
   - [ ] アップデートサーバーの信頼性検証
...
6. **エラーハンドリング**
   - [ ] ネットワークエラー時のリトライ機能
   - [ ] ダウンロード失敗時の再試行
   - [ ] インストール失敗時のエラーメッセージ表示
   - [ ] アップデートログの記録

**技術要件**:
- Tauri Updater Plugin (`tauri-plugin-updater`) を使用
- アップデートサーバー: GitHub Releases API（初期実装）
- 署名方式: Tauriのデフォルト署名検証
- ダウンロード方式: バックグラウンドダウンロード

**UI要件**:
- 設定画面に「アップデート」セクションを追加
- アップデート通知は非侵入的なバッジ表示
- アップデート実行時は進捗ダイアログを表示
- リリースノートはMarkdown形式で表示

**セキュリティ要件**:
- すべてのアップデートは署名検証が必須
- アップデートサーバーはHTTPS通信のみ
- ダウンロードファイルの整合性検証は必須
- アップデートプロセスはユーザーに透明性を提供
...
**機能ID**: F011  
**優先度**: 最高（非開発者体験のため必須）  
**概要**: 初回ユーザー向けのチュートリアルとガイダンス

**詳細仕様**:

1. **初回起動時のオンボーディング**
   - [x] 初回起動時に自動的にオンボーディングを表示
   - [x] オンボーディングのスキップ機能
   - [x] オンボーディングの完了状態をlocalStorageに保存
   - [x] オンボーディングの再表示機能（設定から）

2. **オンボーディングステップ**
   - [x] ステップ1: ようこそメッセージ（FLMの紹介）
   - [x] ステップ2: API作成の説明（「新しいAPIを作成」ボタンのハイライト）
   - [x] ステップ3: モデル管理の説明（「モデル管理」ボタンのハイライト）
   - [x] ステップ4: API管理の説明（「API一覧」ボタンのハイライト）
   - [x] ステップ5: ログと監視の説明
   - [x] ステップ6: ヘルプとサポートの説明

3. **オンボーディングUI機能**
   - [x] プログレスインジケーター（現在のステップ表示）
   - [x] 「次へ」「戻る」「スキップ」ボタン
   - [x] 各ステップでの要素のハイライト表示
   - [x] わかりやすい説明文とアイコン
...
**機能ID**: F010  
**優先度**: 最高（非開発者体験のため必須）  
**概要**: エラー発生時の自動リトライとユーザーフレンドリーなエラーメッセージ

**詳細仕様**:

1. **自動リトライ機能**
   - [x] エラー発生時に自動的にリトライを試行（最大3回）
   - [x] リトライ間隔を段階的に延長（1秒、2秒、3秒）
   - [x] リトライ中の進捗表示（「リトライ中... (1/3)」）
   - [x] すべてのリトライが失敗した場合のみエラーメッセージを表示
   - [x] リトライ可能なエラーの自動判定:
     - ネットワークエラー（接続タイムアウト、DNSエラー等）
     - 一時的なサービスエラー（503、429等）
     - エンジン起動エラー（Ollama、LM Studio等）

2. **エラーメッセージの改善**
   - [x] 技術的なエラーメッセージを非開発者向けに変換
   - [x] 具体的な解決方法を提示
   - [x] エラーの種類に応じたアイコンと色分け
   - [x] ヘルプページへのリンクを自動表示

3. **エラーカテゴリ別の処理**
   - [x] Ollamaエラー: 「Ollamaに接続できませんでした」→ 自動起動を試行
   - [x] ポートエラー: 「ポートが使用中です」→ 別のポート番号を提案
   - [x] モデルエラー: 「モデルが見つかりません」→ モデルダウンロードを提案
   - [x] ネットワークエラー: 「インターネット接続を確認してください」→ 自動リトライ
   - [x] 権限エラー: 「必要な権限がありません」→ トラブルシューティングガイドを表示

4. **トラブルシューティングガイドの自動表示**
   - [x] エラーが解決しない場合、関連するトラブルシューティングガイドを表示
   - [ ] よくある問題と解決方法の一覧を充実
   - [ ] サポートへの問い合わせリンク
...
**機能ID**: F023  
**優先度**: 低（実装済み）  
**概要**: SSL証明書の管理（Let's Encrypt証明書の取得・更新）

**詳細仕様**:

1. **証明書情報表示**
   - [x] 証明書一覧の表示
   - [x] 証明書情報の表示（ドメイン、発行者、有効期限、ステータス）
   - [x] 証明書の有効期限の確認
...
1. **モデルカタログ表示**
   - [x] モデルカタログ一覧の表示
   - [x] モデル情報の詳細表示（名前、サイズ、説明、カテゴリ等）
   - [x] モデルカタログの検索・フィルタリング
...
2. **同期機能**
   - [x] 認証トークンの設定
   - [x] 設定データのクラウドへのアップロード
   - [x] クラウドから設定データのダウンロード
   - [x] 同期状態の表示
   - [x] ローカルファイルバックアップ
...
#### 3.1.3 マルチエンジン対応（v1.0実装済み）

**実装状況**: ✅ **部分実装済み**（v1.0）

**対応エンジン一覧**:

| エンジン | エンジンID | 実装状況 | 初期インストール | 自動アップデート | デフォルトポート | OpenAI互換API |
|---------|-----------|---------|----------------|----------------|----------------|--------------|
| **Ollama** | `ollama` | ✅ 完全実装 | ✅ 自動インストール | ❌ 未実装 | 11434 | ✅ 対応 |
| **LM Studio** | `lm_studio` | ✅ 検出・起動実装 | ❌ 手動インストール必要 | ❌ 未実装 | 1234 | ✅ 対応 |
| **vLLM** | `vllm` | ✅ 検出・起動実装 | ❌ 手動インストール必要 | ❌ 未実装 | 8000 | ✅ 対応 |
| **llama.cpp** | `llama_cpp` | ✅ 検出・起動実装 | ❌ 手動インストール必要 | ❌ 未実装 | 8080 | ⚠️ オプション |
...
1. **エンジン抽象化レイヤー** ✅ 実装済み
   ...
   - 各エンジンの個別実装（`OllamaEngine`, `LMStudioEngine`, `VLLMEngine`, `LlamaCppEngine`）
...
3. **エンジン起動・停止** ✅ 実装済み
   - 自動起動機能（Ollamaのみ完全実装）
   - 手動起動対応（LM Studio、vLLM、llama.cpp）
   - 停止機能（Ollamaのみ完全実装）
...
5. **初期インストール対応** ⚠️ 部分実装
   - **Ollama**: ✅ 自動インストール機能実装済み
     - 初回起動時に未検出の場合、自動ダウンロード・インストール
     - GitHub Releases APIから最新版を取得
     - 進捗表示機能付き
   - **その他エンジン**: ❌ 手動インストール必要
     - LM Studio: 公式サイトから手動インストール
     - vLLM: PythonパッケージまたはDockerコンテナとして手動インストール
     - llama.cpp: システムパスに手動インストール

6. **自動アップデート機能** ❌ 未実装
   - **Ollama**: 
     - バージョン取得機能は実装済み
     - 最新版ダウンロード機能は実装済み（初回インストール時のみ）
     - **既存インストールの自動アップデートは未実装**
   - **その他エンジン**: 
     - バージョン取得機能も未実装
     - アップデート機能も未実装

7. **API作成時のエンジン対応** ✅ 実装済み
   - エンジンタイプの選択機能
   - エンジン別のベースURL設定
   - エンジン状態の確認・自動起動

8. **プロキシサーバーのエンジン対応** ⚠️ 部分実装
   - エンジン別のベースURL設定は実装済み
   - **注意**: プロキシサーバーのエンドポイントがOllama専用になっているため、他のエンジンでは完全に動作しない可能性がある
   - 修正が必要な箇所:
     - `/v1/chat/completions` → Ollama専用エンドポイント（`/api/chat`）を使用
     - 他のエンジン（LM Studio、vLLM、llama.cpp）はOpenAI互換APIのため、直接`/v1/chat/completions`を使用可能

**使用可能な機能**:
- ✅ エンジン一覧の取得
- ✅ エンジンの検出（インストール済みか確認）
- ✅ エンジンの起動（Ollamaは自動、その他は手動起動必要）
- ✅ エンジン別のモデル一覧取得
- ✅ エンジン指定でのAPI作成
- ✅ エンジン設定の保存・取得

**制限事項**:
- ❌ 他のエンジン（LM Studio、vLLM、llama.cpp）は手動インストールが必要
- ❌ エンジンの自動アップデート機能は未実装
- ⚠️ プロキシサーバーがOllama専用のエンドポイントを使用しているため、他のエンジンで完全に動作しない可能性がある

**今後の実装予定**:
- 🔲 プロキシサーバーのエンジン別エンドポイント対応
- 🔲 エンジンの自動アップデート機能（v1.1以降）
- 🔲 他のエンジンの自動インストール機能（v2.0以降）
...
- ✅ ユーザー設定（テーマ、言語、アプリ終了時の動作等） - SQLiteに保存
  - `stop_apis_on_exit`: アプリ終了時にAPIを停止するか（デフォルト: true）
...
- ✅ **カスタムモデル対応**: LLMSTUDIO風の機能を追加（v2.0以降）
  ...
  - 複数形式対応
- ✅ **複数LLM実行エンジン対応**: 
  - **現状（v1.0）**: ✅ マルチエンジン対応の基盤実装済み
    - エンジン抽象化レイヤー実装済み（`LLMEngine`トレイト）
    - エンジンマネージャー実装済み（`EngineManager`）
    - 4つのエンジンに対応（Ollama、LM Studio、vLLM、llama.cpp）
    - エンジン検出・起動・モデル一覧取得機能実装済み
    - **制限**: プロキシサーバーがOllama専用のため、他のエンジンで完全動作しない可能性あり
  - **v1.1予定**: プロキシサーバーのエンジン別エンドポイント対応
  - **v1.3予定**: エンジンの自動アップデート機能
  - **v2.0予定**: 他のエンジンの自動インストール機能
  - **v2.0以降**: カスタムエンドポイント対応、text-generation-webui対応
  - **実装方針**: Strategy/Adapterパターンで実行エンジンを抽象化し、統一インターフェースを提供（✅ 実装済み）
...
1. **LLMSTUDIO風の高度なモデル管理機能** ✅ 実装済み
   - ✅ **カスタムモデル対応**: Hugging Faceなどからの直接ダウンロード・変換（F004参照）
   - ✅ **Modelfile作成支援**: カスタムモデルのModelfileをGUIで作成・編集（F004参照）
   - ✅ **モデル変換機能**: GGUF形式への自動変換（可能な範囲で）（F004参照）
   - ✅ **モデル共有機能**: モデル共有UI実装済み（F004参照、プラットフォーム連携は将来実装予定）
   - ✅ **高度なモデル検索**: Hugging Face APIとの統合による検索拡張（F004参照）

2. **クラウド同期機能** ✅ 実装済み
   - ✅ GitHub Gist同期（F024参照）
   - ✅ Google Drive同期（F024参照）
   - ✅ Dropbox同期（F024参照）
...
5. **その他の実装済み機能**
   - ✅ アラート機能（F013参照）
   - ✅ 監査ログ機能（F014参照）
   - ✅ バックアップ・リストア機能（F015参照）
   - ✅ 証明書管理機能（F023参照）
   - ✅ スケジューラー機能（F019参照）
   - ✅ WebServiceSetup機能（F020参照）
   - ✅ Ollama自動更新機能（F009-EXT参照）

**将来実装予定機能**:
- クラウドデプロイ対応（AWS、Azure、GCPへのデプロイ）
- 複数モデル形式対応（PyTorch、ONNX等の形式への対応拡張）
- モデル共有プラットフォーム連携（Hugging Face Hub、Ollama Hubへの実際のアップロード）
- プラグインの動的ロード機能（.so/.dllファイルからの読み込み）
...
| 1.1.0 | 2024-06-01 | OSS統合方法の詳細化、Tauri推奨選定、Ollama API直接利用方針、MVP範囲の明確化を追記 | - |
| 1.2.0 | 2024-09-01 | 公式Webサイト仕様を追加（HTML/CSS/JavaScriptのみ、静的サイト） | - |
| 1.2.1 | 2024-10-01 | OSS優先の判断基準を明確化（選定プロセス、優先順位、例外条件、品質評価基準を追加） | - |
| 1.2.2 | 2024-11-01 | Ollama自動インストール機能（F009）を追加。非開発者体験のため事前インストール不要に変更。統合方法を更新（自動インストール必須化）。API作成機能にOllama検出・インストールステップを追加 | - |
| 1.2.3 | 2024-12-01 | 公式Webサイト仕様を整理。詳細仕様はDOCKS/公式サイト仕様書.mdを参照 | - |
| 1.2.4 | 2024-12-31 | Ollama自動セットアップ機能（F009）を大幅改善。管理者権限不要のポータブル版方式をデフォルトに変更。非開発者でも簡単に使用できるよう、権限不要・自動実行・わかりやすいエラーメッセージを実装方針として明確化。システムインストールはフォールバック方式に変更。 | - |
| 1.2.5 | 2024-12-31 | Ollama依存リスクの軽減策を追加。アプリバンドル方式を最優先方式に変更（インターネット不要・即座使用可能・バージョン固定による互換性保証）。依存性リスク軽減のため、バージョン固定・互換性レイヤー・フォールバック対応を実装方針に追加。 | - |
| 1.2.6 | 2024-12-31 | モデル管理機能（F004）を大幅強化。LLMSTUDIO風のモデル検索・ダウンロード機能を追加。リアルタイム検索、カテゴリフィルタ、サイズフィルタ、ソート機能、モデルカタログ表示、詳細情報表示、バックグラウンドダウンロードなどの機能を追加。モデルカタログ情報をSQLiteにキャッシュする方式を追加。 | - |
| 1.2.7 | 2024-12-31 | 将来の拡張計画を明確化。v2.0以降でLLMSTUDIO風の高度なモデル管理機能（カスタムモデル対応、Hugging Face統合、Modelfile作成支援、モデル変換機能）を追加予定であることを明記。拡張性の設計方針（抽象化レイヤー、モジュラー設計）を追加。 | - |
| 1.2.8 | 2024-12-31 | 実装済み機能を仕様書に追記。以下の機能を追加: アラート機能（F013）、監査ログ機能（F014）、バックアップ・リストア機能（F015）、OAuth認証機能（F016）、プラグイン管理機能（F018）、スケジューラー機能（F019）、WebServiceSetup機能（F020）、証明書管理機能（F023）、クラウド同期機能（F024）、Ollama自動更新機能（F009-EXT）、ログエクスポート・削除機能（F006-EXT）、モデルカタログ管理機能（F004-EXT）。v2.0セクションを更新し、実装済み機能を明記。 | - |
