...
3. **エンジン確認・インストール**
   - [ ] API作成前に選択されたエンジンのインストール状態を自動確認
   - [ ] **Ollamaの場合**: 未インストール時は自動インストールを実行（F009参照）
   - [ ] **その他エンジンの場合**: 未インストール時は手動インストールを促すメッセージ表示
   - [ ] インストール中の進捗表示（Ollamaのみ）
...
4. **アプリケーション終了時のクリーンアップ**
   - [ ] アプリケーション終了時に、実行中のすべてのAPIを自動的に停止する
   - [ ] 認証プロキシプロセス（Node.jsプロセス）を確実に終了させる
   - [ ] データベースのステータスを「stopped」に更新する
   - [ ] クリーンアップ処理は非同期で実行し、最大10秒のタイムアウトを設定する
   - [ ] クリーンアップ処理中のエラーはログに記録するが、アプリケーション終了をブロックしない
