...
**主要なエラー**:
1. **`src/engines/manager.rs:223`** - `ModelInfo`型が見つからない
   - 問題: `use super::models::ModelInfo;`のインポートが必要
   - 修正方法: `engines/models.rs`から`ModelInfo`をインポート

2. **`src/commands/remote_sync.rs:35`** - 型の不一致
   - 問題: `Result<(), AppError>`が期待されるが、`Result<u32, AppError>`が返されている
   - 修正方法: 戻り値の型を修正

3. **`src/commands/plugin.rs:70`** - `PluginContext`に`plugin_id`フィールドがない
   - 問題: `PluginContext`には`api_id`フィールドのみ存在
   - 修正方法: `plugin_id`を削除するか、`PluginContext`に`plugin_id`フィールドを追加

4. **`src/commands/plugin.rs:74`** - 関数の引数が間違っている
   - 問題: `execute_plugin`は`&PluginInfo`と`&PluginContext`を期待
   - 修正方法: 引数を修正

5. **その他のエラー** - 型不一致、メソッド未定義など
...
1. **コンパイルエラーの修正**
   - 重要度: 🔴 最高
   - 主要な問題:
     - `src/engines/manager.rs:223` - `ModelInfo`型のインポート追加
     - `src/commands/remote_sync.rs:35` - 戻り値の型を修正
     - `src/commands/plugin.rs:70,74` - `PluginContext`の使用を修正
     - その他のエラー
...
1. **`src/engines/manager.rs:223`**
   - エラー: `ModelInfo`型が見つからない
   - 修正方法: `use super::models::ModelInfo;`を追加

2. **`src/commands/remote_sync.rs:35`**
   - エラー: 型の不一致（`Result<(), AppError>`が期待されるが、`Result<u32, AppError>`が返されている）
   - 修正方法: 戻り値の型を修正

3. **`src/commands/plugin.rs:70`**
   - エラー: `PluginContext`に`plugin_id`フィールドがない
   - 修正方法: `plugin_id`を削除するか、`PluginContext`に`plugin_id`フィールドを追加

4. **`src/commands/plugin.rs:74`**
   - エラー: 関数の引数が間違っている（`execute_plugin`は`&PluginInfo`と`&PluginContext`を期待）
   - 修正方法: 引数を修正
