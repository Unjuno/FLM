...
**修正**:
- `crates/flm-cli/tests/integration_test.rs`: `revoked_at.is_some()`でrevoke状態を確認
- `crates/flm-cli/tests/cli_test.rs`: 同様に修正
...
1. `crates/flm-core/src/domain/security.rs` - `ApiKeyMetadata`に`revoked_at`追加
