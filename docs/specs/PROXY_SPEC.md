...
2. **Tor**:
   - SOCKS5 エンドポイントは CLI/UI から `tor://host:port` 形式で渡される（デフォルト `127.0.0.1:9050`）。Proxy 起動時に `tokio::net::TcpStream` によるハンドシェイクを最大3回試行し、全て失敗した場合は `tor_unreachable` イベントを監査ログへ記録したうえで `ProxyError::InvalidConfig` を返す。
