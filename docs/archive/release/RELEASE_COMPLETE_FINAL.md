...
#### テストファイルのlintエラーについて
テストファイル（`tests/`）には71個のエラーと672個の警告がありますが、これらは：
- **リリースには直接影響しません**（テストファイルは本番ビルドに含まれません）
- 主に`@ts-ignore`を`@ts-expect-error`に変更する必要がある
- `require()`を`import`に変更する必要がある
- console.logの警告（テストファイルなので許容範囲内）

**重要**: これらのエラーは実際のアプリケーション動作には影響しません。

---

### 3. 修正したファイル一覧（18ファイル以上）
