...
- **Setup Wizard (External Publish)**: Dashboard から起動できるステッパーで Proxy モード選択 (`dev-selfsigned` / `https-acme`)、SecurityPolicy 設定、Firewall コマンド自動生成を案内。Firewall スクリプトは `docs/SECURITY_FIREWALL_GUIDE.md` に沿って自動/手動適用をサポート
...
- プロキシ起動: `local-http` モード < 2秒、`https-acme` は ACME 処理時間をログ出力し監視
