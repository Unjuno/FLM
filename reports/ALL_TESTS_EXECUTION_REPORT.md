...
**主な失敗カテゴリ:**
- `SystemCheck.test.tsx`: システム情報の表示テストが失敗
- `ModelSelection-auto-start.test.tsx`: エンジン自動起動テストが失敗
